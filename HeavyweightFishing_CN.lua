...
--// 手机/平板屏幕自适应（不修改脚本原有逻辑）
local CN_MainUIScale = Instance.new("UIScale")
CN_MainUIScale.Name = "ResponsiveScale"
CN_MainUIScale.Parent = mainFrame

local function CN_UpdateResponsiveUIScale()
    local cam = Workspace.CurrentCamera
    if not cam or not mainFrame or not mainFrame.Parent then return end
    local vp = cam.ViewportSize
    if vp.X <= 0 or vp.Y <= 0 then return end
    local sx = (vp.X * 0.90) / 700
    local sy = (vp.Y * 0.82) / 480
    CN_MainUIScale.Scale = math.clamp(math.min(sx, sy), 0.60, 0.92)
    mainFrame.AnchorPoint = Vector2.new(0.5, 0.5)
    mainFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
end
CN_UpdateResponsiveUIScale()
...
--// 中文界面翻译层：只修改界面显示文字，不修改配置键、变量名和内部逻辑
local CN_UI_PHRASES = {
    ...
    pcall(function()
        for _, obj in ipairs(root:GetDescendants()) do
            if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                local old = obj.Text
                local newText = CN_TranslateText(old)
                if newText ~= old then obj.Text = newText end
