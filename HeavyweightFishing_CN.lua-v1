...
local ConfigLabelMap = {
    ...
    ["自动猛击鱼竿"] = "AutoSlam",
    ["自动蓄力鱼线"] = "AutoCharge",
    ["自动防卡竿"] = "AntiStuckEnabled",

    -- Smart Combo
    ["启用自动技能连招"] = "SmartComboEnabled",
    ["鱼血量判断阈值"] = "FishHpThreshold",
    ["快速收鱼技能"] = "QuickCatchSkill",
    ["起手技能"] = "OpenerSkill",
    ["起手技能使用次数"] = "OpenerMaxCount",
    ["循环技能序列"] = "LoopSkills",
    ["自定义技能序列"] = "LoopSkills",
    ["技能序列预设"] = "LoopSkills",
    ["严格按顺序连招"] = "LoopStrictOrder",
    ["回血/保命技能"] = "EmergencyHealSkill",
    ["低于此HP时回血"] = "EmergencyHealHp",
    ["技能释放延迟"] = "SkillEffectDelay",
    ["自动识别技能效果"] = "SmartEffectAutoDetect",

    -- Auto Luyện Chiêu
    ["启用自动练技能"] = "AutoTrainSkill",
    ["选择要练习的技能"] = "TrainSkill",
    ["技能取消延迟"] = "TrainCancelDelay",
    ["目标使用次数"] = "TrainTargetCount",

    -- Bán cá & Bảo vệ
    ["背包满时自动卖鱼"] = "AutoSell",
    ["自动卖鱼间隔"] = "SellInterval",
    ["自动锁定变异鱼"] = "AutoProtectMutations",
    ["自动收集制作材料鱼"] = "MaterialFarming",
    ["自动锁定喜欢的鱼"] = "AutoFavouriteFish",
    ["要锁定的鱼名"] = "FavouriteFishName",

    -- Tự trang bị
    ["自动装备最佳鱼竿"] = "AutoEquipBestRod",
    ["Tự Dùng Cần Tốt Nhất"] = "AutoEquipBestRod",
    ["自动装备鱼饵"] = "AutoEquipBestBait",
    ["Tự Dùng Mồi (Auto Bait)"] = "AutoEquipBestBait",
    ["Tự Dùng Mồi Tốt Nhất"] = "AutoEquipBestBait",
    ["Chọn Mồi Khi Câu Thường"] = "BaitChoiceNormal",
    ["狩猎Boss时自动换饵"] = "AutoEquipBossBait",
    ["Boss鱼饵选择"] = "BaitChoiceBoss",
    ["自动装备最佳法宝"] = "AutoEquipBestOrb",
    ["自动使用最佳宝珠"] = "AutoEquipBestOrb",
    ...
    ["自动寻找天气服务器"] = "AutoWeatherHop",
    ["目标天气"] = "TargetWeather",
    ["找到后自动钓鱼/狩猎Boss"] = "WeatherHopAutoFish",
    ["找到服务器后发送Webhook"] = "WeatherHopAlertWebhook",

    -- Thần linh
    ["自动检查神灵状态"] = "AutoGodSpiritCheck",
    ["自动向神灵祈祷"] = "AutoPrayGodSpirit",
    ["换服寻找神灵"] = "AutoServerHopGod",
    ["换服寻找Maoshan"] = "AutoServerHopMaoshan",
    ["换服寻找道士（Taoist）"] = "AutoServerHopTaoist",
    ...
    ["玩家 ESP"] = "ESP_Players",
    ["鱼类定位圈"] = "FishRedRing",
    ["在红圈显示重量与变异"] = "ShowFishWeightRing",
    ["远距离视野"] = "ClearFarVision",
    ["移除雾气与暴雨"] = "NoFog",
    ["高亮屏幕"] = "Fullbright",
    ["亮度等级"] = "FullbrightLevel",
    ["天气防眩光"] = "FullbrightAntiGlare",
    ["低画质降延迟"] = "PerformanceMode",
    ["隐藏游戏原生UI"] = "HideGameUI",
    ["隐藏玩家默认名称"] = "HideOverheadNames",

    -- Nhân vật
    ["提高移动速度"] = "WalkSpeedEnabled",
    ["移动速度"] = "WalkSpeedValue",
    ["自由飞行"] = "FlyEnabled",
    ["飞行速度"] = "FlySpeed",
    ["无限跳"] = "InfiniteJump",
    ...
    ["启用Webhook"] = "WebhookEnabled",
    ["Boss捕获通知"] = "WebhookNotifyBoss",
...
--         task.spawn(function()
--             task.wait(1.5)
--             ShowNotification("CẤU HÌNH TỰ ĐỘNG", "已 nạp cài đặt thiết yếu (Skill, Combo, Tiện ích) từ máy!", "SUCCESS", 5)
--         end)
--     end
-- end)
...
--// 手机/平板屏幕自适应：根据当前 Viewport 自动缩放 UI，避免界面过大或超出屏幕
local mainUIScale = Instance.new("UIScale")
mainUIScale.Name = "ResponsiveScale"
mainUIScale.Scale = 0.92
mainUIScale.Parent = mainFrame

local function UpdateResponsiveUIScale()
    local cam = Workspace.CurrentCamera
    if not cam then return end
    local vp = cam.ViewportSize
    if vp.X <= 0 or vp.Y <= 0 then return end

    -- 设计尺寸：700x480；左右留约 10%，上下留约 18%
    local sx = (vp.X * 0.90) / 700
    local sy = (vp.Y * 0.82) / 480
    local scale = math.min(sx, sy, 0.92)
...
table.insert(activeConnections, Workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function()
    task.defer(UpdateResponsiveUIScale)
end))
...
task.spawn(function()
    ...
    while isRunning and mainFrame and mainFrame.Parent do
        ...
        if cam then
            local vp = cam.ViewportSize
            if vp.X ~= lastX or vp.Y ~= lastY then
                lastX, lastY = vp.X, vp.Y
                UpdateResponsiveUIScale()
            end
        end
        task.wait(0.25)
...
brandTitle.Text = "钓鱼助手 PRO"; brandTitle.TextColor3 = Colors.PurplePrimary
...
gameSubtitle.Text = "HEAVYWEIGHT FISHING | 中文版"; gameSubtitle.TextColor3 = Colors.PurpleMuted
...
searchBox.BackgroundColor3 = Colors.InputBg; searchBox.Font = Enum.Font.Gotham; searchBox.PlaceholderText = "搜索功能..."
...
local footerBrand = Instance.new("TextLabel"); footerBrand.Size = UDim2.new(0, 260, 1, 0); footerBrand.Position = UDim2.new(0, 12, 0, 0); footerBrand.BackgroundTransparency = 1; footerBrand.Font = Enum.Font.Gotham; footerBrand.Text = "Heavyweight Fishing | 中文版 V1.1"; footerBrand.TextColor3 = Colors.TextMuted; footerBrand.TextSize = 10; footerBrand.TextXAlignment = Enum.TextXAlignment.Left; footerBrand.Parent = footerBar
local footerKey = Instance.new("TextLabel"); footerKey.Size = UDim2.new(0, 280, 1, 0); footerKey.Position = UDim2.new(1, -292, 0, 0); footerKey.BackgroundTransparency = 1; footerKey.Font = Enum.Font.Gotham; footerKey.Text = "[右Ctrl] 菜单 | [End] 关闭脚本"; footerKey.TextColor3 = Colors.TextMuted; footerKey.TextSize = 10; footerKey.TextXAlignment = Enum.TextXAlignment.Right; footerKey.Parent = footerBar
...
killBtn.MouseButton1Click:Connect(function()
    ShowNotification("关闭脚本", "正在断开连接并完全关闭脚本……", "WARN", 2)
    task.wait(0.2)
    UnloadScript()
end)
...
local function createCollapsibleCardGroup(parent, text, defaultOpen)
    ...
    badge.Text = isOpen and "▼ 收起" or "▶ 展开"
...
function Wiki.UnlockBaitFish(baitName, silent)
    local pData = ReplicatedStorage:FindFirstChild("Data") and ReplicatedStorage.Data:FindFirstChild(LocalPlayer.UserId)
    if not pData then
        if not silent then ShowNotification("解锁鱼饵", "未找到背包数据！", "WARN", 4) end
        return 0
    end
    ...
    if #toUnlock == 0 then
        if not silent then
            ShowNotification("解锁鱼饵", string.format("背包中没有被锁定的 %s 材料鱼。", bLabel), "INFO", 4)
        end
        return 0
    end
    ...
    if not silent then
        ShowNotification("MỞ KHÓA MỒI THÀNH CÔNG", string.format("已解锁 %d 条鱼 làm 鱼饵 %s! Bạn có thể chế tạo ngay.", unlockedCount, (baitName and baitName ~= "All") and ("[" .. baitName .. "]") or "3 Loại 鱼饵"), "SUCCESS", 6)
    end
...
function Wiki.LockBaitFish(baitName, silent)
    local pData = ReplicatedStorage:FindFirstChild("Data") and ReplicatedStorage.Data:FindFirstChild(LocalPlayer.UserId)
    if not pData then
        if not silent then ShowNotification("重新锁定鱼饵", "未找到背包数据！", "WARN", 4) end
        return 0
    end
    ...
    if #toLock == 0 then
        if not silent then
            ShowNotification("重新锁定鱼饵", string.format("背包中的所有 %s 材料鱼都已安全锁定。", bLabel), "INFO", 4)
        end
        return 0
    end
    ...
    if not silent then
        ShowNotification("KHÓA MỒI THÀNH CÔNG", string.format("已保护锁定 lại %d con cá làm 鱼饵 %s 安全 trước AutoSell!", lockedCount, (baitName and baitName ~= "All") and ("[" .. baitName .. "]") or "3 Loại 鱼饵"), "SUCCESS", 6)
    end
    return lockedCount
end
...
function Wiki.UnlockAllKeepFish()
    local pData = ReplicatedStorage:FindFirstChild("Data") and ReplicatedStorage.Data:FindFirstChild(LocalPlayer.UserId)
    if not pData then
        ShowNotification("解锁背包", "未找到玩家背包数据！", "WARN", 4)
        return 0
    end
    ...
    if #toUnlock == 0 then
        ShowNotification("解锁珍稀鱼", "背包中没有需要保留且已被锁定的鱼。", "INFO", 4)
        return 0
    end
    ...
    ShowNotification("MỞ KHÓA THÀNH CÔNG", string.format("已解锁 %d 条鱼 珍稀 / boss / 材料!", unlockedCount), "WARN", 6)
    return unlockedCount
end
...
function Wiki.UnlockAllFish()
    local pData = ReplicatedStorage:FindFirstChild("Data") and ReplicatedStorage.Data:FindFirstChild(LocalPlayer.UserId)
    if not pData then
        ShowNotification("解锁", "未找到背包数据！", "WARN", 4)
        return 0
    end
    ...
    if #toUnlock == 0 then
        ShowNotification("全部解锁", "背包中没有被锁定的鱼。", "INFO", 4)
        return 0
    end
    ...
    ShowNotification("MỞ KHÓA TẤT CẢ", string.format("已解锁 %d 条鱼 (kể cả cá 珍稀)! AutoSell có thể bán 全部.", count), "SUCCESS", 6)
    return count
end
...
function Wiki.UnlockAllUnnecessaryFish()
    ...
    if #toUnlock == 0 then
        ShowNotification("解锁背包", "背包中没有不需要保留且已被锁定的鱼。", "INFO", 4)
        return 0
    end
    ...
    ShowNotification("MỞ KHÓA THÀNH CÔNG", string.format("已解锁 %d 条鱼 không cần thiết! AutoSell có thể bán ngay.", unlockedCount), "SUCCESS", 6)
    return unlockedCount
end
...
function Wiki.LockAllKeepFish()
    ...
    local pData = ReplicatedStorage:FindFirstChild("Data") and ReplicatedStorage.Data:FindFirstChild(LocalPlayer.UserId)
    if not pData then
        ShowNotification("保护锁定", "未找到背包数据！", "WARN", 4)
        return 0
    end
    ...
    if #toLock == 0 then
        ShowNotification("保护锁定", "背包中所有需要保留的鱼都已安全锁定。", "INFO", 4)
        return 0
    end
    ...
    ShowNotification("KHÓA THÀNH CÔNG", string.format("已保护锁定 安全 %d con cá 珍稀 / boss / 材料!", lockedCount), "SUCCESS", 6)
    return lockedCount
end
...
function Wiki.ToggleLockSpecificFish(fishName, targetKeepState)
    ...
    if #itemsFound == 0 then
        ShowNotification("鱼类操作", string.format("背包中没有 [%s] 可切换状态。", fishName), "INFO", 4)
        return
    end
    ...
    if toggled > 0 then
        ShowNotification("鱼类操作", string.format("%s %d con [%s] 成功!", actText, toggled, fishName), "SUCCESS", 5)
    else
        ShowNotification("鱼类操作", string.format("[%s] 已经全部处于 %s 状态。", fishName, wantLock and "khóa" or "mở khóa"), "INFO", 4)
    end
end
...
local function TrackCaughtFishForGems(child)
    ...
    if reward > 0 then
        ...
        ShowNotification("Gems 奖励", string.format("捕获 %s! 获得 +%d Gems!", fishName, reward), "SUCCESS", 4)
    end
end
...
local function ServerHop(avoidServers)
    ShowNotification("切换服务器", "正在寻找合适的服务器……", "WARN")
    ...
    task.spawn(function()
        ...
        if chosen then
            ShowNotification("切换服务器", "已找到新服务器！正在传送……", "SUCCESS", 3)
            task.wait(0.5)
        ...
        ShowNotification("切换服务器", "正在切换到随机服务器……", "INFO", 3)
        task.wait(0.5)
        pcall(function() TeleportService:Teleport(placeId, LocalPlayer) end)
    end)
end
...
secretBossState.HandleTeleportError = function(reason)
    ...
    if secretBossState.isHopping then
        ...
        ShowNotification("切换服务器", "服务器 已满 hoặc 错误 (" .. tostring(reason or "GameFull"):sub(1, 35) .. "). 自动 tìm 服务器 khác...", "WARN", 4)
    ...
    elseif secretBossState.isNormalHopping then
        ShowNotification("切换服务器", "服务器已满或连接错误，正在自动尝试其他服务器……", "WARN", 4)
        task.delay(1.5, function()
...
function secretBossState.HopToNextWeatherServer(targetWeather, visitedServers)
    ...
    ShowNotification("寻找服务器", string.format("正在扫描 服务器 còn 空位 cho: %s...", targetWeather), "WARN", 5)
    ...
    task.spawn(function()
        ...
        if foundJob then
            ...
            ShowNotification("切换服务器", "已选择有空位的服务器！正在传送……", "SUCCESS", 4)
            task.wait(0.5)
            ...
            task.delay(10, function()
                if secretBossState.isHopping and Config.AutoWeatherHop and secretBossState.hopWatchdog == myWatchdog and game.JobId == myOrigJob then
                    ...
                    ShowNotification("切换服务器", "无法 vào 服务器 (服务器 已满 / hết hạn). 自动 tìm 服务器 khác...", "WARN", 4)
                    task.wait(1)
            ...
            if not teleOk then
                table.insert(visitedServers, foundJob)
                ShowNotification("切换服务器", "错误 连接 (" .. tostring(teleErr):sub(1, 40) .. "). 正在 尝试 服务器 khác...", "WARN", 4)
                task.wait(1.5)
                if secretBossState.isHopping and Config.AutoWeatherHop then
                    secretBossState.HopToNextWeatherServer(targetWeather, visitedServers)
                end
            ...
            if not isRunning or not secretBossState.isHopping or not Config.AutoWeatherHop then return end
            ShowNotification("寻找服务器", "没有找到有空位的服务器，正在切换到随机服务器……", "INFO", 4)
            task.wait(1.5)
            if not isRunning or not secretBossState.isHopping or not Config.AutoWeatherHop then return end
            pcall(function() TeleportService:Teleport(placeId, LocalPlayer) end)
        end
...
function secretBossState.CheckWeatherHopOnJoin()
    ...
    ShowNotification("寻找服务器", string.format("正在扫描 thời tiết 服务器 cho: %s...", targetWeather), "INFO", 5)
    ...
    task.delay(3.0, function()
        ...
        if isMatch then
                ...
                task.delay(1.5, function()
                    if matchedEntry and matchedEntry.pos then
                        ...
                        if root then
                            ...
                            ShowNotification("Boss 狩猎", "已 tự bay đến " .. matchedEntry.islandName .. " để 钓鱼 / săn boss!", "SUCCESS", 5)
                        end
                    end
            ...
            ShowNotification("寻找服务器", string.format("天气 hiện tại: %s (Không khớp). Tiếp tục đổi 服务器...", curDisplay), "WARN", 3)
            task.wait(1.5)
            if not isRunning or not secretBossState.isHopping or not Config.AutoWeatherHop then return end
...
function secretBossState.CheckNPCHopOnJoin()
    ...
    ShowNotification("切换服务器", "正在检查新服务器中的 NPC / 神灵……", "INFO", 5)
end
...
function secretBossState.ReturnToHome()
    ...
    if dist > 35 then
        ...
        ShowNotification("VỀ VỊ TRÍ FARM", "天气晴朗 / Boss 已消失！正在返回挂机位置钓鱼赚钱……", "INFO", 5)
...
function secretBossState.Teleport(matchedIsland, detectedName, reqPower)
    ...
    if Config.SecretBossCheckPower and powerReq > 0 and curPower < powerReq then
        ShowNotification("鱼竿力量警告", string.format("Boss yêu cầu %d Power! Cần của bạn chỉ có %d Power.", powerReq, curPower), "WARN", 8)
    end
    ...
    ShowNotification("PHÁT HIỆN " .. alertName .. "!", string.format("正在 bay đến %s để 钓鱼 boss...", matchedIsland.islandName), "SUCCESS", 7)
    ...
    if root and matchedIsland.pos then
        ...
        if customCf then
            ...
            ShowNotification("VỊ TRÍ TÙY CHỌN", string.format("已 vào [位置 %d/%d]%s tại %s!", chosenSlot or 1, totalSpots or 1, jitterTag, matchedIsland.islandName), "SUCCESS", 5)
        else
            ...
            standPos = targetCFrame.Position
            if foundWater then
                ShowNotification("钓鱼水域", string.format("已找到水域！角色已到达岸边钓鱼位置：%s.", matchedIsland.islandName), "SUCCESS", 5)
            end
        end
...
function secretBossState.HandleChatMessage(msg)
    ...
    if (lower:find("all secret bosses") and (lower:find("despawn") or lower:find("gone") or lower:find("disappear")))
       ...
       or lower:find("kết thúc") then
        ...
        ShowNotification("Secret Boss 消失", "全部 Secret Boss 已 biến mất! Hệ thống 正在 等待 đợt xuất hiện tiếp theo.", "WARN", 7)
        if statusLabelSecretBoss and statusLabelSecretBoss.Set then
        ...
        if Config.AutoServerHopOnDespawn then
            ShowNotification("自动换服务器", "Secret Boss 已 hết! 正在 tự động đổi 服务器 để săn tiếp...", "WARN", 5)
            task.delay(1.5, function()
                if Config.AutoServerHopOnDespawn then
                    ServerHop()
                end
        ...
        if matchedIsland and detectedName ~= "Clear" then
            ...
            if Config.PrioritySystemEnabled and PriorityManager and PriorityManager.GetActiveTask then
                ...
                if curTask ~= "None" and curTask ~= "SecretBoss" and PriorityManager.GetTaskPriority(curTask) < bossPri then
                    ShowNotification("Ưu Tiên", string.format("Phát hiện %s tại %s nhưng 正在 ưu tiên [%s] (Hạng %d)!", tostring(detectedName), matchedIsland.islandName, PriorityManager.GetTaskDisplayName(curTask), PriorityManager.GetTaskPriority(curTask)), "WARN", 5)
                    return
                end
            end
...
function ticketQuestState.IsAllQuestsDoneToday()
    ...
    if isDateChanged or isServerDailyReset then
        ...
        ShowNotification("新的一天 - 重置任务票", "新的一天开始！机器人自动返回 NPC 领取任务票。", "SUCCESS", 6)
        return false
    end
    return true
end
...
function ticketQuestState.HandleDialogue(isClaiming)
    ...
    if isDoneToday then
        ...
        ticketQuestState.UpdateUI()
        ShowNotification("任务已完成", "NPC: 已用完 全部 任务票 任务 hôm nay! Hẹn gặp lại ngày mai.", "WARN", 8)
        if leaveBtn then
    ...
    local function _execute()
        ...
        if ticketQuestState.IsDialogueOpen() then
            if ticketQuestState.HandleDialogue(isClaiming) then
                if isClaiming then
                    ShowNotification("任务票", "任务提交成功并获得任务票奖励！", "SUCCESS", 5)
                else
                    ShowNotification("任务票", "已成功领取任务 " .. (isHard and "Hard" or "Easy") .. " Ticket!", "SUCCESS", 5)
                end
                return true
            end
            task.wait(0.4)
...
function ticketQuestState.ResetCooldown()
    ...
    ticketQuestState.UpdateUI()
    ShowNotification("任务票", "已 xóa 冷却, cờ hết 任务票 và đặt lại bộ đếm!", "SUCCESS", 5)
end
        ...
        if Config.TicketReturnHomeWhenDone and Config.HomeFarmSpot and not ticketQuestState.isAtHomeSpot then
            ...
            if root then
                ...
                if homeCf then
                    ...
                    ShowNotification("Home Spot", "今日任务票已用完，已返回 Home Spot 挂机！", "SUCCESS", 5)
                ...
                if Config.TicketReturnHomeWhenDone and Config.HomeFarmSpot and not ticketQuestState.isAtHomeSpot then
                    ...
                    if root then
                        ...
                        if homeCf then
                            ...
                            ShowNotification("Home Spot", "等待任务票期间已返回 Home Spot 挂机钓鱼！", "SUCCESS", 5)
    ...
    if isDoneNow then
        if now - (ticketQuestState.lastClaimAttempt or 0) >= 3.5 then
            ...
            if questCleared or (serverCd > nowServer) or claimSuccess then
                ...
                ticketQuestState.SaveSpots()
                ticketQuestState.UpdateUI()
                ShowNotification("任务票", string.format("已提交 任务票 Hard! 正在 等待 冷却 (%d 分钟).", math.ceil(setCooldown / 60)), "SUCCESS", 8)
...
local tabBoss      = CreateTab("Boss狩猎")
local tabFishManager = CreateTab("鱼类管理")
local tabGod       = CreateTab("神灵")
local tabQuests    = CreateTab("任务")
local tabShop      = CreateTab("商店与制作鱼饵")
local tabTeleports = CreateTab("传送")
local tabVisuals   = CreateTab("ESP与画质")
local tabPlayer    = CreateTab("角色")
local tabProfiles  = CreateTab("设置")
local tabExperimental = CreateTab("实验功能")

SwitchTab("Câu Cá")


createCategoryHeader(tabFishing, "账户信息与统计")
...
createButtonRow(statsCard, "Đặt Lại Thông Số Treo (Reset AFK)", "Đặt lại giờ treo và tính lại tốc độ cá/tiền chính xác từ mốc này", "🔄 Reset Thông Số", function()
    ...
    ShowNotification("Thống Kê Treo", "已 đặt lại mốc thời gian và tính lại tốc độ 钓鱼/tiền từ thời điểm này!", "SUCCESS", 4)
end)"挂机统计", "Đã đặt lại mốc thời gian và tính lại tốc độ câu/tiền từ thời điểm này!", "SUCCESS", 4)
end)

createCategoryHeader(tabFishing, "核心自动钓鱼")
local fishCard = createCardGroup(tabFishing)

createToggleRow(fishCard, "自动 Chơi Mini Game (Auto Minigame)", "Tự động thắng mọi minigame (Kéo cần, Perfect Slam, Max Charge & Rhythm Boss Bạch Tuộc)", Config.AutoMinigame, function(v)
...
createSliderRow(fishCard, "Tỉ Lệ Trúng Minigame (Accuracy %)", "Độ chính xác khi gõ nhịp Boss (Khuyên dùng 92% - 96% để 安全 chống soi anti-cheat)", 70, 100, Config.RhythmAccuracy or 95, false, "%", function(v)
    Config.RhythmAccuracy = v
end)
createToggleRow(fishCard, "真人模式（拟人化时间）", "Giả lập độ trễ phản xạ 10-35ms và vị trí bấm lệch 随机 như tay người thật", Config.RhythmHumanizer, function(v)
    Config.RhythmHumanizer = v
end)
createButtonRow(fishCard, "🧪 Test Thử Minigame Bạch Tuộc (A-S-D)", "Bật giao diện 3 làn A-S-D và thả nốt rơi 尝试 nghiệm để kiểm chứng bot tự bấm Perfect ngay trước mắt", "Bấm Để Test", function()
    task.spawn(function()
        local pg = LocalPlayer:FindFirstChild("PlayerGui")
        ...
        if not rhythm then
            ShowNotification("错误 Test", "未找到 giao diện Rhythm trong game!", "WARN", 4)
            return
        end
        ...
        rhythm.Visible = true
        ShowNotification("🧪 正在 Test", "Giao diện A-S-D 已 mở! 正在 tạo nốt rơi 尝试 nghiệm...", 3)
            ...
            if prog and prog.Visible then
                ...
                task.delay(0.9, function()
                    if testNote and testNote.Parent then
                        ...
                        ShowNotification("✨ PERFECT!", "Bot 已 tự động bấm nốt làn [" .. laneData.key .. "]!", 2)
        ...
        ShowNotification("🎉 Test Hoàn Tất", "已 kiểm chứng bot tự bấm nốt 成功 100%!", 4)
    end)
end)"Lỗi Test", "未找到 giao diện Rhythm trong game!", "WARN", 4)
            return
        end
        local origFVis = fishing and fishing.Visible
        local origRVis = rhythm.Visible
...
createToggleRow(fishCard, "自动 Quăng Cần (Auto Cast)", "Tự động bắt đầu câu và quăng cần 持续", Config.AutoCast, function(v) Config.AutoCast = v end)
createSliderRow(fishCard, "Độ Trễ Quăng Cần", "Thời gian giãn cách giữa các 次 quăng", 0.0, 5.0, Config.CastDelay, true, "s", function(v) Config.CastDelay = v end)
...
createToggleRow(fishCard, "Tự Dùng Kỹ Năng Cần", "Tự kích hoạt 技能 cần câu để kéo cá siêu 快速", Config.AutoSkills, function(v) Config.AutoSkills = v end)
createToggleRow(fishCard, "自动 Đập Cần (Auto Slam)", "Tự động nhấn Slam mức Perfect khi xuất hiện", Config.AutoSlam, function(v) Config.AutoSlam = v end)
createToggleRow(fishCard, "自动 Sạc Dây (Auto Charge)", "Tự động sạc đầy 100% độ bền dây câu", Config.AutoCharge, function(v) Config.AutoCharge = v end)
createToggleRow(fishCard, "自动 Chống Kẹt Cần (Anti-Stuck)", "Tự động phát hiện và gỡ kẹt khi quăng cần hoặc minigame bị đơ quá 15s", Config.AntiStuckEnabled, function(v) Config.AntiStuckEnabled = v end)

createCategoryHeader(tabFishing, "🏠 Boss狩猎结束返回位置")
...
local infoHomeSpot = createInfoRow(returnSpotCard, "返回位置 Hiện Tại", GetHomeSpotText())

createToggleRow(returnSpotCard, "Tự Về 位置 Này Khi Hết Boss / Clear", "Khi hết Boss hoặc thời tiết Clear, tự bay về vị trí này câu cá farm tiền", Config.ReturnToHomeWhenClear, function(v)
    Config.ReturnToHomeWhenClear = v
end)

createToggleRow(returnSpotCard, "Tự Về 位置 Này Khi Xong 任务票 NV", "Khi xong vé nhiệm vụ và vào 20p chờ, tự bay về vị trí này", Config.TicketReturnHomeWhenDone, function(v)
...
createButtonRow(returnSpotCard, "保存位置 正在 Đứng Làm Điểm Trở Về", "Lưu tọa độ & hướng quay hiện tại làm nơi Farm cá mặc định (lưu riêng theo tài khoản)", "保存位置", function()
    local ok, spot = secretBossState.SaveHomeSpot()
    if ok then
        if infoHomeSpot and infoHomeSpot.Set then
            infoHomeSpot.Set(GetHomeSpotText())
        end
        ShowNotification("位置 Trở Về", "已 lưu vị trí trở về 成功 cho tài khoản này!", "SUCCESS", 5)
    else
        ShowNotification("错误 Lưu", tostring(spot), "ERROR")
    end
end)"返回位置", "已保存 vị trí trở về thành công cho tài khoản này!", "SUCCESS", 5)
    else
        ShowNotification("错误 Lưu", tostring(spot), "ERROR")
    end
end)

createButtonRow(returnSpotCard, "Bay Về Điểm Trở Về Ngay", "传送 tức thì về vị trí trở về 已 lưu và 开始 钓鱼", "Bay Về", function()
    if not Config.HomeFarmSpot then
        ShowNotification("Chưa Lưu 位置", "请 đứng tại nơi muốn 钓鱼 rồi bấm [Lưu 位置] trước!", "WARN", 5)
        return
    end
    ...
    ShowNotification("位置 Trở Về", "已 bay về vị trí trở về mặc định!", "SUCCESS", 4)
end)"Chưa 保存位置", "请 đứng tại nơi muốn câu rồi bấm [保存位置] trước!", "WARN", 5)
        return
    end
    secretBossState.lastHomeReturnTime = 0
    local ok = secretBossState.ReturnToHome()
    ...
    ShowNotification("返回位置", "已飞到 về vị trí trở về mặc định!", "SUCCESS", 4)
end)

createButtonRow(returnSpotCard, "Xóa Điểm Trở Về 已 Lưu", "Xóa vị trí trở về của tài khoản này", "Xóa 位置", function()
    secretBossState.ClearHomeSpot()
    if infoHomeSpot and infoHomeSpot.Set then
        infoHomeSpot.Set(GetHomeSpotText())
    end
    ShowNotification("位置 Trở Về", "已 xóa vị trí trở về của tài khoản này.", "INFO")
end)"返回位置", "Đã xóa vị trí trở về của tài khoản này.", "INFO")
end)

createCategoryHeader(tabFishing, "⚔️ 智能技能连招")
local comboCard = createCardGroup(tabFishing)

createToggleRow(comboCard, "Bật Combo Kỹ Năng 自动", "Tự động kích hoạt chiêu theo ngưỡng máu cá, chiêu mở màn và đảo chiêu luân phiên", Config.SmartComboEnabled, function(v)
    Config.SmartComboEnabled = v
    SaveSmartCombo()
end)

createSliderRow(comboCard, "Ngưỡng Máu Cá Phân Loại", "Máu cá <= mức này sẽ kết liễu 快速; > mức này sẽ bật combo", 100, 3000, Config.FishHpThreshold, false, " HP", function(v)
    Config.FishHpThreshold = v
    SaveSmartCombo()
end)

createDropdownRow(comboCard, "技能 Bắt 快速 (<= Ngưỡng HP)", "Tung 1 hit kết liễu ngay khi cá yếu / cá 普通", {"Tắt", "Z", "X", "C", "V"}, Config.QuickCatchSkill, function(v)
    Config.QuickCatchSkill = v
    SaveSmartCombo()
end)

createDropdownRow(comboCard, "技能 Mở Màn (> Ngưỡng HP)", "技能 tung 1 次 duy nhất đầu trận khi gặp cá to / boss", {"Tắt", "Z", "X", "C", "V"}, Config.OpenerSkill, function(v)
    Config.OpenerSkill = v
    SaveSmartCombo()
end)

createSliderRow(comboCard, "Số 次 Dùng 技能 Mở Màn", "Số 次 tung chiêu mở màn trước khi chuyển sang đảo chiêu", 1, 3, Config.OpenerMaxCount, false, " lần", function(v)
...
do
    ...
    presetDropdown = createDropdownRow(comboCard, "Mẫu Chuỗi 技能 (Preset)", "Chọn 快速 chuỗi phổ biến hoặc tự do tùy biến ở dưới", loopPresets, getPresetLabel(Config.LoopSkills), function(v)
...
createDropdownRow(comboCard, "技能 Hồi Máu / Cứu Nguy", "Ưu tiên tung chiêu này khi máu người chơi xuống thấp", {"Tắt", "Z", "X", "C", "V"}, Config.EmergencyHealSkill, function(v)
    Config.EmergencyHealSkill = v
    SaveSmartCombo()
end)

createSliderRow(comboCard, "激活 Hồi Máu Khi HP Dưới", "Ngưỡng máu người chơi cần cứu nguy khẩn cấp", 10, 80, Config.EmergencyHealHp, false, "%", function(v)
    Config.EmergencyHealHp = v
    SaveSmartCombo()
end)

createSliderRow(comboCard, "时间 Chờ Ra 技能", "Thời gian tối thiểu chờ hết hiệu ứng trước khi tung chiêu tiếp theo", 0.1, 2.5, Config.SkillEffectDelay, true, "s", function(v)
    Config.SkillEffectDelay = v
    SaveSmartCombo()
end)

createToggleRow(comboCard, "自动 Nhận Diện Hết Hiệu Ứng", "Quan sát hoạt ảnh đòn đánh trên nhân vật để chống nuốt chiêu 100%", Config.SmartEffectAutoDetect, function(v)
    Config.SmartEffectAutoDetect = v
    SaveSmartCombo()
end)

createCategoryHeader(tabFishing, "🎯 快速自动练技能")
local trainCard = createCardGroup(tabFishing)
local infoTrainProgress = createInfoRow(trainCard, "Tiến Độ Luyện 技能", string.format("%d / %d 次", tonumber(Config.TrainCurrentCount) or 0, tonumber(Config.TrainTargetCount) or 100))
createToggleRow(trainCard, "Bật Auto Luyện 技能", "Cá cắn kéo là dùng chiêu -> cất cần hủy cá -> thả cần lại ngay", Config.AutoTrainSkill, function(v) Config.AutoTrainSkill = v end)
createDropdownRow(trainCard, "Chọn 技能 Cần Luyện", "Chọn 1 chiêu duy nhất muốn luyện (Z, X, C, V)", {"Z", "X", "C", "V"}, Config.TrainSkill or "Z", function(v) Config.TrainSkill = v end)
createSliderRow(trainCard, "Nhịp Chờ Xuất 技能 (Cancel Delay)", "Thời gian chờ nhân vật bắt đầu xuất chiêu trước khi cất cần (0.2s - 1.2s)", 0.2, 1.2, Config.TrainCancelDelay or 0.45, true, "s", function(v)
    Config.TrainCancelDelay = v
end)
createSliderRow(trainCard, "Mục Tiêu Số 次 Dùng", "Số 次 cần dùng để đạt yêu cầu tiến hóa (mặc định 100 次)", 10, 500, Config.TrainTargetCount, false, " lần", function(v)
...
local function ExportAllPlayerSkills(infoRow, ownedOnly)
    ...
    if #skillList > 0 then
        if copyOk then
            ShowNotification("导出技能", string.format("已 quét %d 技能 và sao chép vào Clipboard!", #skillList), "SUCCESS", 6)
        else
            ShowNotification("导出技能", string.format("已 quét %d 技能! 正在 mở bảng xem trực tiếp...", #skillList), "SUCCESS", 6)
        end
    else
        ShowNotification("导出技能", "Chưa thấy 技能 nào! Hãy mở Menu Skill trong game rồi bấm lại nhé.", "WARN", 6)
    end
...
do
createCategoryHeader(tabFishing, "自动装备优化")
...
createToggleRow(equipCard, "狩猎 Boss 时自动切换鱼饵", "Tự động đổi sang mồi săn boss tối ưu khi vào chế độ Boss 狩猎", Config.AutoEquipBossBait, function(v)
    Config.AutoEquipBossBait = v
end)

createDropdownRow(equipCard, "选择 Boss 狩猎鱼饵", "Loại mồi ưu tiên sử dụng khi săn Boss", baitOptionsList, Config.BaitChoiceBoss, function(v)
    Config.BaitChoiceBoss = v
end)

createToggleRow(equipCard, "自动使用鱼饵", "Tự động móc loại mồi đã chọn khi câu cá bình 普通", Config.AutoEquipBestBait, function(v)
    Config.AutoEquipBestBait = v
end)

createDropdownRow(equipCard, "选择普通钓鱼鱼饵", "Loại mồi sử dụng cho câu cá thông 普通", baitOptionsList, Config.BaitChoiceNormal, function(v)
    Config.BaitChoiceNormal = v
end)

createToggleRow(equipCard, "自动使用最佳鱼竿", "Tự động cầm cần câu có chỉ số lực mạnh nhất bạn sở hữu", Config.AutoEquipBestRod, function(v) Config.AutoEquipBestRod = v end)
createToggleRow(equipCard, "自动装备最佳宝石", "Tự động trang bị viên Ngọc có cấp bậc cao nhất", Config.AutoEquipBestOrb, function(v) Config.AutoEquipBestOrb = v end)
...
createDropdownRow(equipCard, "套装 1：鱼竿", "Chọn cần câu cho Bộ Set 1", rodNameList, Config.Loadout1_Rod, function(v) Config.Loadout1_Rod = v end)
createDropdownRow(equipCard, "套装 1：鱼饵", "Chọn mồi câu cho Bộ Set 1", baitNameList, Config.Loadout1_Bait, function(v) Config.Loadout1_Bait = v end)
createButtonRow(equipCard, "快速装备套装 1", "Trang bị Cần & 鱼饵 已 chọn cho Set 1", "使用套装 1", function()
    local pData = ReplicatedStorage:FindFirstChild("Data") and ReplicatedStorage.Data:FindFirstChild(LocalPlayer.UserId)
    if pData then
        ...
        if isOwned then
            ...
            ShowNotification("Bộ Set #1", "已装备 cần: " .. Config.Loadout1_Rod, "SUCCESS")
        else
            ShowNotification("Bộ Set #1", "你尚未拥有 cần: " .. Config.Loadout1_Rod, "WARN")
        end
        ...
        if bFolder and bFolder.Value > 0 then
            ...
            ShowNotification("Bộ Set #1", "已装备 鱼饵: " .. Config.Loadout1_Bait, "SUCCESS")
        else
            ShowNotification("Bộ Set #1", "库存中有 0 " .. Config.Loadout1_Bait .. "!", "WARN")
        end
    end
end)"Bộ Set #1", "已装备 cần: " .. Config.Loadout1_Rod, "SUCCESS")
        else
            ShowNotification("Bộ Set #1", "你尚未拥有 cần: " .. Config.Loadout1_Rod, "WARN")
        end
        local bFolder = pData.Bait:FindFirstChild(Config.Loadout1_Bait)
...
createDropdownRow(equipCard, "套装 2：鱼竿", "Chọn cần câu cho Bộ Set 2", rodNameList, Config.Loadout2_Rod, function(v) Config.Loadout2_Rod = v end)
createDropdownRow(equipCard, "套装 2：鱼饵", "Chọn mồi câu cho Bộ Set 2", baitNameList, Config.Loadout2_Bait, function(v) Config.Loadout2_Bait = v end)
createButtonRow(equipCard, "快速装备套装 2", "Trang bị Cần & 鱼饵 已 chọn cho Set 2", "使用套装 2", function()
    local pData = ReplicatedStorage:FindFirstChild("Data") and ReplicatedStorage.Data:FindFirstChild(LocalPlayer.UserId)
    if pData then
        ...
        if isOwned then
            ...
            ShowNotification("Bộ Set #2", "已装备 cần: " .. Config.Loadout2_Rod, "SUCCESS")
        else
            ShowNotification("Bộ Set #2", "你尚未拥有 cần: " .. Config.Loadout2_Rod, "WARN")
        end
        ...
        if bFolder and bFolder.Value > 0 then
            ...
            ShowNotification("Bộ Set #2", "已装备 鱼饵: " .. Config.Loadout2_Bait, "SUCCESS")
        else
            ShowNotification("Bộ Set #2", "库存中有 0 " .. Config.Loadout2_Bait .. "!", "WARN")
        end
    end
end)"Bộ Set #2", "已装备 cần: " .. Config.Loadout2_Rod, "SUCCESS")
        else
            ShowNotification("Bộ Set #2", "你尚未拥有 cần: " .. Config.Loadout2_Rod, "WARN")
        end
        local bFolder = pData.Bait:FindFirstChild(Config.Loadout2_Bait)
...
createCategoryHeader(tabFishing, "经济与自动卖鱼")
local sellCard = createCardGroup(tabFishing)
createToggleRow(sellCard, "自动卖鱼", "Tự động bán 全部 cá 在背包中 theo 周期", Config.AutoSell, function(v) Config.AutoSell = v end)
createSliderRow(sellCard, "卖鱼间隔时间", "Chu kỳ số 秒 tự động bán cá 1 次", 10, 300, Config.SellInterval, false, "s", function(v) Config.SellInterval = v end)

createButtonRow(sellCard, "立即出售并飞往 Nana", "传送 tức thì đến NPC Nana và bán 全部 cá", "立即出售", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root then
        ...
        if Events and Events:FindFirstChild("SellFish") then
            Events.SellFish:FireServer("All")
            ShowNotification("卖鱼", "已 bán toàn bộ cá cho NPC Nana 成功!", "SUCCESS")
        end
    end
end)"卖鱼", "已出售 全部 cá cho NPC Nana thành công!", "SUCCESS")
        end
    end
end)
...
createToggleRow(sellCard, "锁定珍稀鱼（自动收藏）", "保护 cá 珍稀 hiếm đã chọn, không bao giờ bị bán nhầm", Config.AutoFavouriteFish, function(v) Config.AutoFavouriteFish = v end)
createDropdownRow(sellCard, "选择要锁定的鱼", "Loại cá cần 保护 không bán", fishList, Config.FavouriteFishName, function(v) Config.FavouriteFishName = v end)
createToggleRow(sellCard, "自动锁定变异鱼", "Tự động khóa mọi cá Shiny, Giant, Golden, Albino, Corrupted", Config.AutoProtectMutations, function(v) Config.AutoProtectMutations = v end)
createToggleRow(sellCard, "材料刷取模式", "Giữ lại cá làm nguyên liệu, không bán", Config.MaterialFarming, function(v) Config.MaterialFarming = v end)
end

do
createCategoryHeader(tabBoss, "🌩️ 天气图腾")
local totemCard = createCardGroup(tabBoss)

for _, t in ipairs(weatherTotems) do
    createButtonRow(totemCard, t.name, "飞往 và kích hoạt: " .. t.weather, "激活", function()
        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if root then
            root.CFrame = CFrame.new(t.pos + Vector3.new(0, 3, 0))
            ShowNotification("祭坛 天气", "已到达 " .. t.name .. "! 正在 tương tác...", "SUCCESS", 4)
            task.wait(0.4)
            pcall(function()
    ...
    end)"天气图腾", "Đã đến " .. t.name .. "! Đang tương tác...", "SUCCESS", 4)
            task.wait(0.4)
            pcall(function()
                for _, d in ipairs(Workspace:GetDescendants()) do
                    if d:IsA("ProximityPrompt") and (d.Parent:IsA("BasePart") or d.Parent:IsA("Model")) then
...
(function()
    createCategoryHeader(tabBoss, "⚡ 自动寻找天气服务器")
    local weatherHopCard = createCardGroup(tabBoss)

    secretBossState.weatherHopToggle = createToggleRow(weatherHopCard, "自动寻找天气服务器", "Tự động đổi server 持续 bằng queue_on_teleport đến khi gặp đúng thời tiết", Config.AutoWeatherHop, function(v)
        ...
        if v then
            ...
            if isMatch then
                ShowNotification("寻找服务器", string.format("服务器 hiện tại 已 có thời tiết: %s!", weatherName or Config.TargetWeather), "SUCCESS", 5)
                Config.AutoWeatherHop = false
                secretBossState.isHopping = false
                if secretBossState.weatherHopToggle and secretBossState.weatherHopToggle.Set then
                ...
                ShowNotification("寻找服务器", string.format("开始 搜索 服务器 có: %s...", Config.TargetWeather), "WARN", 5)
                task.wait(0.8)
                if secretBossState.isHopping and Config.AutoWeatherHop then
            ...
            ShowNotification("寻找服务器", "已 tắt 搜索 服务器 thời tiết.", "INFO", 4)
        end
    end)"寻找服务器", string.format("Server hiện tại đã có thời tiết: %s!", weatherName or Config.TargetWeather), "SUCCESS", 5)
                Config.AutoWeatherHop = false
                secretBossState.isHopping = false
                if secretBossState.weatherHopToggle and secretBossState.weatherHopToggle.Set then
                    secretBossState.weatherHopToggle.Set(false, true)
    ...
    createDropdownRow(weatherHopCard, "选择要寻找的天气", "Loại thời tiết mục tiêu bạn muốn săn lùng", secretBossState.weatherHopChoices, Config.TargetWeather, function(v)
        Config.TargetWeather = v
    end)

    createToggleRow(weatherHopCard, "找到后自动钓鱼 / 狩猎 Boss", "Tự động bay đến đảo có thời tiết đó và bật AutoCast / Boss 狩猎", Config.WeatherHopAutoFish, function(v)
        Config.WeatherHopAutoFish = v
    end)

    createToggleRow(weatherHopCard, "找到服务器后发送 Webhook", "Gửi thông báo JobId và 信息 server về Discord Webhook", Config.WeatherHopAlertWebhook, function(v)
        Config.WeatherHopAlertWebhook = v
    end)

    createButtonRow(weatherHopCard, "立即停止搜索", "Hủy bỏ quá trình nhảy 服务器 và xóa dữ liệu ghi nhớ", "Dừng Tìm", function()
        secretBossState.isHopping = false
        Config.AutoWeatherHop = false
        secretBossState.hopWatchdog = (secretBossState.hopWatchdog or 0) + 1
        pcall(function()
        ...
        ShowNotification("寻找服务器", "已 dừng và hủy bỏ quá trình 搜索!", "INFO", 4)
    end)"寻找服务器", "Đã dừng và hủy bỏ quá trình tìm kiếm!", "INFO", 4)
    end)
end)()

createCategoryHeader(tabBoss, "秘密章鱼 Boss（Octoparasite）")
local octoCard = createCardGroup(tabBoss)

createToggleRow(octoCard, "Tự Chơi Minigame (Rhythm Bot)", "Bot tự động gõ nhịp 安全", Config.OctoAutoMinigame, function(v) Config.OctoAutoMinigame = v end)
createSliderRow(octoCard, "Tỉ Lệ Trúng Nhịp Điệu (Accuracy %)", "Độ chính xác khi gõ nhịp (92% - 96% giúp tài khoản 安全 tuyệt đối)", 70, 100, Config.RhythmAccuracy or 95, false, "%", function(v)
    Config.RhythmAccuracy = v
end)
createToggleRow(octoCard, "真人模式（拟人化时间）", "Giả lập độ trễ phản xạ tự nhiên 10-35ms", Config.RhythmHumanizer, function(v)
    Config.RhythmHumanizer = v
end)
createButtonRow(octoCard, "飞往 Phao Boss Bạch Tuộc", "传送 đến phao triệu hồi Secret Boss giữa biển", "飞往", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root then
        root.CFrame = CFrame.new(1608.2, 5.0, -218.3)
        ShowNotification("Dịch Chuyển", "已到达 Phao Boss Bạch Tuộc!", "SUCCESS")
    end
end)"传送", "Đã đến Phao Boss Bạch Tuộc!", "SUCCESS")
    end
end)
createButtonRow(octoCard, "飞往 Vùng Lòng Đất", "传送 đến vùng đất 钓鱼 ngầm 秘密", "飞往", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root then
        root.CFrame = CFrame.new(112.5, -330.0, -30.8)
        ShowNotification("Dịch Chuyển", "已到达 Vùng 钓鱼 Cá Ngầm!", "SUCCESS")
    end
end)"传送", "Đã đến Vùng 钓鱼 Ngầm!", "SUCCESS")
    end
end)

createCategoryHeader(tabBoss, "🎯 Secret Boss狩猎与筛选")
local chatBossCard = createCardGroup(tabBoss)

createToggleRow(chatBossCard, "Bật 制作 Độ Boss 狩猎 (Tự Quăng Cần & Lọc Cá)", "Tự động quăng cần và giật bỏ cá 普通, chỉ câu trúng Boss mục tiêu", Config.AutoHuntBoss, function(v)
    ...
    if v then
        ...
        ShowNotification("Boss 狩猎", "已 BẬT Chế Độ Boss 狩猎! Tự quăng cần và giật bỏ cá thường.", "SUCCESS", 5)
    else
        ...
        ShowNotification("Boss 狩猎", "已 TẮT Chế Độ Boss 狩猎.", "INFO")
    end
end)"Boss 狩猎", "Đã BẬT 制作 Độ Boss 狩猎! Tự quăng cần và giật bỏ cá 普通.", "SUCCESS", 5)
    else
        if not Config.AutoChatSecretBoss then
            secretBossState.active = false
        end
        ...
        ShowNotification("Boss 狩猎", "已 TẮT 制作 Độ Boss 狩猎.", "INFO")
    end
end)

createToggleRow(chatBossCard, "自动 Bay Theo Chat (Chat Sniper)", "Tự nghe tin nhắn chat server, khi có boss thì tự bay đến đảo có boss", Config.AutoChatSecretBoss, function(v)
    Config.AutoChatSecretBoss = v
    if v then
        ShowNotification("Chat Sniper", "正在 lắng nghe thông báo Boss từ chat 服务器...", "SUCCESS", 4)
        task.spawn(function()
            task.wait(0.3)
...
end)"Chat Sniper", "Đang lắng nghe thông báo Boss từ chat server...", "SUCCESS", 4)
        task.spawn(function()
            task.wait(0.3)
            local found = secretBossState.ScanChatHistory()
            if not found then
...
createToggleRow(chatBossCard, "Bỏ Qua Cá 普通 (Fast Skip)", "Nếu cắn câu không phải Secret Boss đã chọn thì lập tức giật cần thả lại", Config.FastSkipNonBoss, function(v)
    Config.FastSkipNonBoss = v
end)

createToggleRow(chatBossCard, "检查 Lực Cần (Power Check)", "Cảnh báo nếu cần câu hiện tại không đủ lực yêu cầu của Boss", Config.SecretBossCheckPower, function(v)
    Config.SecretBossCheckPower = v
end)

createToggleRow(chatBossCard, "Boss 消失后自动换服务器", "Tự động đổi server khác ngay khi có thông báo tất cả Secret Boss đã despawn", Config.AutoServerHopOnDespawn, function(v)
    Config.AutoServerHopOnDespawn = v
end)

createToggleRow(chatBossCard, "显示 Boss 伤害面板（% HP）", "Hiển thị bảng DPS thời gian thực tính % máu và sát thương từng người chơi khi cùng pem Boss", Config.ShowBossDpsMeter, function(v)
    Config.ShowBossDpsMeter = v
end)

statusLabelSecretBoss = createInfoRow(chatBossCard, "状态 Săn:", secretBossState.statusText)

createButtonRow(chatBossCard, "Quét Lại Lịch Sử Chat & Boss", "检查 lại lịch sử chat xem có Boss nào 正在 hoạt động không", "Quét Chat", function()
    local found = secretBossState.ScanChatHistory()
    if not found then
        ShowNotification("Kết Quả Quét", "未找到 Secret Boss nào 正在 hoạt động trong lịch sử chat.", "INFO", 5)
    end
end)"Kết Quả Quét", "未找到 Secret Boss nào đang hoạt động trong lịch sử chat.", "INFO", 5)
    end
end)

createButtonRow(chatBossCard, "Dò Tìm & 飞往 Mép Nước", "自动 quét tia 360 độ tìm vùng nước và đưa nhân vật ra sát mép bờ 钓鱼", "Dò Mép Nước", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root then
        ...
        if found then
            ...
            ShowNotification("Mép Nước", "已找到 vùng nước và bay ra sát mép bờ 钓鱼!", "SUCCESS", 5)
            CancelAndRecastRod()
        else
            ShowNotification("Mép Nước", "未找到 vùng nước trong bán kính 220 studs!", "WARN", 5)
        end
    end
end)"Mép Nước", "Đã tìm thấy vùng nước và bay ra sát mép bờ câu!", "SUCCESS", 5)
            CancelAndRecastRod()
        else
            ShowNotification("Mép Nước", "未找到 vùng nước trong bán kính 220 studs!", "WARN", 5)
        end
...
createButtonRow(chatBossCard, "Chọn Tất Cả Secret Boss", "Bật săn 全部 các loài Secret Boss trên mọi đảo", "Chọn Hết", function()
    ...
    SaveBossTargets()
    ShowNotification("Secret Boss", "已选择 全部 Secret Boss!", "SUCCESS")
end)"Secret Boss", "Đã chọn tất cả Secret Boss!", "SUCCESS")
end)

createButtonRow(chatBossCard, "Bỏ Chọn Tất Cả", "Tắt săn 全部 Secret Boss", "Bỏ Hết", function()
    for bName, _ in pairs(Config.SecretBossTargets) do
    ...
    SaveBossTargets()
    ShowNotification("Secret Boss", "已 bỏ chọn 全部 Secret Boss.", "INFO")
end)"Secret Boss", "Đã bỏ chọn tất cả Secret Boss.", "INFO")
end)

do
    -- CÀI ĐẶT VỊ TRÍ CÂU TÙY CHỌN (CUSTOM FISHING SPOTS - HỖ TRỢ 3 ĐIỂM/ĐẢO + XÊ DỊCH)
    createCategoryHeader(tabBoss, "📍 自定义钓鱼位置")
    ...
    local infoSavedSpots = createInfoRow(customSpotCard, "Tiến Độ 已 Cài:", string.format("%d / %d đảo (Tổng %d vị trí)", islCount, #islandNamesList, spCount))
    ...
    local slotDropdown = createDropdownRow(customSpotCard, "Chọn 位置 (Slot 1 - 3)", "每 đảo có thể lưu tối đa 3 vị trí câu khác nhau", slotOptions, "Vị Trí " .. tostring(currentSlot), function(v)
    ...
    infoCurrentSlot = createInfoRow(customSpotCard, "状态 位置:", GetSlotStatusText(currentIsland, currentSlot))
    ...
    createDropdownRow(customSpotCard, "Phân Bổ 位置 Khi Boss 狩猎", "Nhiều acc cùng server sẽ tự chia nhau các vị trí khác nhau", allocOptions, Config.BossSpotAllocationMode or allocOptions[1], function(v)
    ...
    createButtonRow(customSpotCard, "⚡ TỰ NHẬN DIỆN & LƯU VÀO VỊ TRÍ TRỐNG TIẾP THEO", "Đứng ở mép nước trên đảo, script tự biết đảo và lưu vào vị trí trống (1 -> 2 -> 3)!", "LƯU ĐIỂM TIẾP THEO", function()
        ...
        if nearestEntry then
            ...
            ShowNotification("ĐÃ LƯU VỊ TRÍ CÂU!", string.format("已 lưu [位置 %d] cho %s! (Đảo này 已 có %d/3 vị trí)", targetSlot, targetIsland, totalSaved), "SUCCESS", 7)
        else
            ShowNotification("错误 Nhận Diện", "Không xác định được đảo gần nhất!", "ERROR", 5)
        end
    end)"ĐÃ LƯU VỊ TRÍ CÂU!", string.format("已保存 [位置 %d] cho %s! (Đảo này đã có %d/3 vị trí)", targetSlot, targetIsland, totalSaved), "SUCCESS", 7)
        else
            ShowNotification("错误 Nhận Diện", "未知 được đảo gần nhất!", "ERROR", 5)
        end
    end)

    createButtonRow(customSpotCard, "Lưu Vào Đảo & 位置 正在 Chọn", "Lưu tọa độ & hướng nhìn hiện tại vào chính xác Slot 正在 chọn ở trên", "Lưu Vào Slot Này", function()
        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if not root then return end
        ...
        ShowNotification("ĐÃ LƯU VỊ TRÍ CÂU!", string.format("已 lưu [位置 %d] cho %s! (Đảo này 已 có %d/3 vị trí)", currentSlot, currentIsland, totalSaved), "SUCCESS", 6)
    end)"ĐÃ LƯU VỊ TRÍ CÂU!", string.format("已保存 [位置 %d] cho %s! (Đảo này đã có %d/3 vị trí)", currentSlot, currentIsland, totalSaved), "SUCCESS", 6)
    end)

    createButtonRow(customSpotCard, "Bay Thử 位置 正在 Chọn", "飞往 vị trí 正在 chọn để kiểm tra (có kèm xê dịch nếu 正在 bật)", "Bay Thử", function()
        ...
        if root and spot and spot.cframe then
            ...
            ShowNotification("Bay Thử 位置", string.format("已 bay đến [位置 %d] của [%s]%s!", currentSlot, currentIsland, jitterMsg), "SUCCESS", 5)
        else
            ShowNotification("Chưa Cài Đặt", string.format("Bạn chưa lưu [位置 %d] cho [%s]!", currentSlot, currentIsland), "WARN", 5)
        end
    end)"Bay Thử 位置", string.format("已飞到 đến [位置 %d] của [%s]%s!", currentSlot, currentIsland, jitterMsg), "SUCCESS", 5)
        else
            ShowNotification("Chưa 设置", string.format("Bạn chưa lưu [位置 %d] cho [%s]!", currentSlot, currentIsland), "WARN", 5)
        end
    end)

    createButtonRow(customSpotCard, "Xóa 位置 正在 Chọn", "Xóa chỉ riêng vị trí (Slot) 正在 chọn này của đảo", "Xóa Slot Này", function()
        secretBossState.DeleteIslandSlot(currentIsland, currentSlot)
        RefreshSpotUI()
        ShowNotification("已 Xóa 位置", string.format("已 xóa [位置 %d] của [%s].", currentSlot, currentIsland), "INFO", 4)
    end)"Đã Xóa 位置", string.format("Đã xóa [位置 %d] của [%s].", currentSlot, currentIsland), "INFO", 4)
    end)

    createButtonRow(customSpotCard, "Xóa Hết Cả 3 位置 Của Đảo Này", "Xóa 全部 các vị trí 已 lưu của đảo 正在 chọn để dùng dò tìm mép nước gốc", "Xóa Cả Đảo", function()
        secretBossState.DeleteIslandSlot(currentIsland, 0)
        RefreshSpotUI()
        ShowNotification("已 Xóa Hết", string.format("已 xóa toàn bộ vị trí tùy chọn của [%s].", currentIsland), "INFO", 4)
    end)"Đã Xóa Hết", string.format("Đã xóa 全部 vị trí tùy chọn của [%s].", currentIsland), "INFO", 4)
    end)

    createButtonRow(customSpotCard, "📋 COPY TOÀN BỘ TỌA ĐỘ (ĐỂ NẠP VÀO SCRIPT GỐC)", "Copy 全部 2-3 vị trí 已 cài của 全部 các đảo ra mã Lua để dán vào code gốc cho TẤT CẢ mọi người dùng chung", "COPY TỌA ĐỘ", function()
        local lines = {}
        ...
        if copied then
            ShowNotification("ĐÃ COPY VÀO CLIPBOARD!", string.format("已 copy tọa độ của %d đảo (%d vị trí)! Dán vào chat với AI để nạp vào script gốc cho 全部 mọi người dùng chung.", totalIslands, totalSpots), "SUCCESS", 8)
        else
            ShowNotification("Xuất Tọa Độ", "已 in mã tọa độ ra bảng điều khiển Console F9! Hãy mở F9 để copy.", "INFO", 6)
        end
    end)"Chưa Có Tọa Độ", "Bạn chưa cài tọa độ cho đảo nào cả! Hãy đi đến các đảo và bấm Lưu trước.", "WARN", 5)
            return
        end
        local fullCode = table.concat(lines, "\n")
        print("\n======== [TỌA ĐỘ VỊ TRÍ CÂU SĂN BOSS EXPORT] ========\n" .. fullCode .. "\n====================================================\n")
...
-- ====================================================================
do
createCategoryHeader(tabBoss, "🎣 鱼竿制作指南")
...
createButtonRow(rodGuideCard, "Bật Lại Tất Cả Secret Boss", "Bật lại 全部 secret boss sau khi 已 lọc theo cần 钓鱼", "↩ Bật Hết Lại", function()
    for bName, _ in pairs(Config.SecretBossTargets) do
        Config.SecretBossTargets[bName] = true
        if bossTogglesMap[bName] and bossTogglesMap[bName].Set then
            pcall(function() bossTogglesMap[bName].Set(true, true) end)
    ...
    SaveBossTargets()
    ShowNotification("已 Bật Hết", "已 bật lại toàn bộ secret boss!", "SUCCESS")
end)"Đã Bật Hết", "Đã bật lại 全部 secret boss!", "SUCCESS")
end)
end

do
    createCategoryHeader(tabBoss, "Enzo Boss竞技场")
    local bossFarmCard = createCardGroup(tabBoss)
    createToggleRow(bossFarmCard, "自动 Boss 狩猎 (Enzo)", "Liên tục triệu hồi và đánh bại boss Enzo", Config.AutoFarmBoss, function(v) Config.AutoFarmBoss = v end)
    ...
    createButtonRow(bossFarmCard, "飞往 Boss Enzo", "传送 trực tiếp đến đấu trường Enzo", "飞往", function()
        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if root then
            root.CFrame = CFrame.new(-115.3, 9.2, 1349.5)
            ShowNotification("Dịch Chuyển", "已到达 Đấu trường Boss Enzo!", "SUCCESS")
        end
    end)"传送", "Đã đến Đấu trường Boss Enzo!", "SUCCESS")
        end
    end)
end
...
do
    ...
    local function ProcessBatchItems(items, targetLock, notifyTitle)
        ...
        if FM.isProcessing then
            if tick() - (FM.lastProcessTime or 0) > 4 then
                ...
                ShowNotification("鱼类管理", "正在 trong tiến trình xử lý, vui lòng đợi!", "WARN", 2)
                return
            end
        end
        ...
        if #filtered == 0 then
            ShowNotification(notifyTitle or "鱼类管理", "全部 cá trong danh sách 已 ở trạng thái mong muốn!", "INFO", 3)
            return
        end
        ...
        ShowNotification(notifyTitle or "鱼类管理", string.format("开始 %s %d con cá...", actionText, #filtered), "INFO", 3)
        ...
        task.spawn(function()
            ...
            ShowNotification("Hoàn Tất", string.format("已 %s 成功 %d/%d con cá!", actionText, successCount, #filtered), "SUCCESS", 4)
        end)
    end
    ...
    local function SellSpecificFish(targetFishName, quantityToSell)
        if FM.isProcessing then
            ShowNotification("卖鱼", "正在 bận xử lý tác vụ khác, vui lòng đợi!", "WARN", 3)
            return
        end
        ...
        if not cData or #cData.items == 0 then
            ShowNotification("卖鱼", "未找到 con [" .. tostring(targetFishName) .. "] nào 在背包中!", "WARN", 4)
            return
        end
        ...
        FM.isProcessing = true
        FM.lastProcessTime = tick()
        ShowNotification("卖鱼 An Toàn", string.format("正在 chuẩn bị bán %d con [%s]...", #targetItems, targetFishName), "INFO", 3)
        ...
        task.spawn(function()
            local ok, err = pcall(function()
                ...
                if sellEvent then
                    ...
                    ShowNotification("卖鱼 成功", string.format("已出售 %d con [%s]! Cá khác được giữ 安全 100%%!", #targetItems, targetFishName), "SUCCESS", 5)
                else
                    ShowNotification("错误 卖鱼", "未找到 Remote SellFish!", "ERROR", 4)
                end
            end)
    ...
    do
        ...
        FM.rowBagTotal = createInfoRow(bagOverviewCard, "Tổng cá trong 背包", "正在扫描……")
        FM.rowBagProtected = createInfoRow(bagOverviewCard, "Cá 珍稀 正在 保护", "...")
        FM.rowBagJunk = createInfoRow(bagOverviewCard, "Cá rác có thể bán 安全", "...")

        createButtonRow(bagOverviewCard, "锁定 Toàn Bộ Cá 珍稀", "保护 tuyệt đối cá Boss, Cần, 鱼饵, VIP, Đột Biến", "🔒 Khóa Cá 珍稀", function()
            local protList, _, _ = ScanAndClassifyInventory()
            ProcessBatchItems(protList, true, "Bảo Vệ Cá Quý")
        end)

        createButtonRow(bagOverviewCard, "解锁 Riêng Cá Rác", "Chỉ mở khóa các con cá rác thông 普通", "🔓 Mở Khóa Rác", function()
            local _, junkList, _ = ScanAndClassifyInventory()
            ProcessBatchItems(junkList, false, "Mở Khóa Cá Rác")
        end)

        createButtonRow(bagOverviewCard, "出售 Sạch Cá Rác An Toàn", "自动 kiểm tra 安全 và bán 全部 cá rác", "💰 出售 Sạch Rác", function()
            if FM.isProcessing then
                ShowNotification("Dọn Rác", "正在 bận xử lý tác vụ khác, vui lòng đợi!", "WARN", 3)
                return
            end

            local protList, junkList, _ = ScanAndClassifyInventory()
            if #junkList == 0 then
                ShowNotification("Dọn Rác", "没有 cá rác nào 在背包中 cần bán!", "INFO", 3)
                return
            end

            FM.isProcessing = true
            ShowNotification("Dọn Rác", string.format("正在 dọn dẹp %d con cá rác an toàn...", #junkList), "INFO", 4)
            ...
            task.spawn(function()
                ...
                if sellEvent then
                    sellEvent:FireServer("All")
                    ShowNotification("卖鱼 Thành Công", string.format("已 bán sạch %d con cá rác! Cá quý bảo vệ 100%%!", #junkList), "SUCCESS", 5)
                else
                    ShowNotification("错误 卖鱼", "未找到 Remote SellFish!", "ERROR", 4)
                end
        ...
        end)"Dọn Rác", "Đang bận xử lý tác vụ khác, vui lòng đợi!", "WARN", 3)
                return
            end
            ...
            FM.isProcessing = true
            ShowNotification("Dọn Rác", string.format("正在 dọn dẹp %d con cá rác 安全...", #junkList), "INFO", 4)
            ...
            task.spawn(function()
                ...
                if sellEvent then
                    sellEvent:FireServer("All")
                    ShowNotification("卖鱼 成功", string.format("已出售 sạch %d con cá rác! Cá 珍稀 保护 100%%!", #junkList), "SUCCESS", 5)
                else
                    ShowNotification("错误 卖鱼", "未找到 Remote SellFish!", "ERROR", 4)
                end
    ...
    do
        ...
        searchTextBox.PlaceholderText = "输入要查找的鱼名（例如：crim、carp、koi、eel、catfish...）"
        ...
        fishStatusLbl.Text = "匹配关键词的鱼类会显示在上方，点击即可选择。"
        ...
        function FM.UpdateSearchedFishUI(targetQuery)
            ...
            if #matchedNames > 0 then
                ...
                fishStatusLbl.Text = "背包中没有匹配关键词的鱼。"
                fishImgIcon.Image = ""
                fishEmojiIcon.Visible = true
            end
        end
        ...
        btnLockThis.MouseButton1Click:Connect(function()
            if not FM.currentSelectedFish then
                ShowNotification("Tìm Kiếm", "请 chọn một loài cá hợp lệ trước!", "WARN", 3)
                return
            end
            local _, _, counts = ScanAndClassifyInventory()
        ...
        btnSellSpecific.MouseButton1Click:Connect(function()
            if not FM.currentSelectedFish then
                ShowNotification("卖鱼", "请 chọn hoặc tìm một loài cá hợp lệ trước khi bán!", "WARN", 4)
                return
            end
    ...
    ---------------------------------------------------------------------
    do
        createCategoryHeader(tabFishManager, "🎣 鱼竿制作进度")

        for _, rDef in ipairs(ROD_RECIPES) do
            local rCard = createCardGroup(tabFishManager)

            local rowHeader = createInfoRow(rCard, rDef.vietName, "正在扫描……")
            local rowProgress = createInfoRow(rCard, "Tiến độ 材料", "0/4 (0%)")
            local grid = CreateVisualIngredientGrid(rCard, rDef.ingredients)

            createButtonRow(rCard, "锁定 4 Nguyên Liệu", "保护 tuyệt đối 4 con cá làm cần " .. rDef.key, "🔒 Khóa 4 NL", function()
                local _, _, counts = ScanAndClassifyInventory()
                local toLock = {}
            ...
            createButtonRow(rCard, "解锁 An Toàn", "解锁 cá cần nhưng giữ lại cá trùng 鱼饵 thần thoại", "🔓 Mở Khóa An Toàn", function()
            ...
            end)"Cảnh Báo", "Bạn CHƯA sở hữu " .. rDef.key .. "! Cân nhắc trước khi mở khóa!", "WARN", 5)
                end
                local _, _, counts = ScanAndClassifyInventory()
                local toUnlock = {}
                for _, ing in ipairs(rDef.ingredients) do
    ...
    ---------------------------------------------------------------------
    do
        createCategoryHeader(tabFishManager, "🍖 神话鱼饵制作材料")

        for _, bDef in ipairs(BAIT_RECIPES) do
            local bCard = createCardGroup(tabFishManager)
            local rowHeader = createInfoRow(bCard, bDef.vietName, "正在扫描……")
            local grid = CreateVisualIngredientGrid(bCard, bDef.ingredients)

            createButtonRow(bCard, "锁定 4 Nguyên Liệu 鱼饵", "保护 全部 4 loài cá làm 鱼饵 " .. bDef.key, "🔒 Khóa Cá Mồi", function()
                local _, _, counts = ScanAndClassifyInventory()
                local toLock = {}
            ...
            createButtonRow(bCard, "解锁 Để 制作 鱼饵", "解锁 4 loài cá này kèm cờ chống tự khóa", "🔓 解锁鱼饵", function()
                local _, _, counts = ScanAndClassifyInventory()
                local toUnlock = {}
                for _, ing in ipairs(bDef.ingredients) do
                    local ingLower = ing.name:lower():gsub("[%s%-_]+", "")
    ...
    do
        ...
        createButtonRow(singleCard, "锁定 " .. TARGET_TEST_FISH, "锁定 全部 con " .. TARGET_TEST_FISH .. " 正在 mở", "🔒 Khóa", function()
            local _, _, counts = ScanAndClassifyInventory()
            local cData = counts[TARGET_TEST_FISH]
            if cData and #cData.items > 0 then
                ProcessBatchItems(cData.items, true, "Khóa " .. TARGET_TEST_FISH)
            else
                ShowNotification("实验功能", "没有 con " .. TARGET_TEST_FISH .. " nào 在背包中!", "WARN", 3)
            end
        end)"实验功能", "Không có con " .. TARGET_TEST_FISH .. " nào 在背包中!", "WARN", 3)
            end
        end)

        createButtonRow(singleCard, "解锁 " .. TARGET_TEST_FISH, "解锁 全部 con " .. TARGET_TEST_FISH .. " 正在 khóa", "🔓 Mở Khóa", function()
            local _, _, counts = ScanAndClassifyInventory()
            local cData = counts[TARGET_TEST_FISH]
            if cData and #cData.items > 0 then
        ...
        createButtonRow(singleCard, "Bật Spy Bắt Remote", "Bấm nút này rồi mở 背包 click vào ngôi sao của 1 con cá", "🔍 Bật Spy", function()
            if FM.spyEnabled then
                ShowNotification("SPY REMOTE", "Spy 已 được bật! Hãy mở 背包 và click vào biểu tượng Ngôi Sao của bất kỳ con cá nào.", "INFO", 5)
                return
            end
            FM.spyEnabled = true
            ...
            ShowNotification("SPY ĐÃ KÍCH HOẠT", "Bây giờ hãy mở 背包 Game và click vào biểu tượng Ngôi Sao của 1 con cá bất kỳ!", "SUCCESS", 6)
        end)"SPY REMOTE", "Spy đã được bật! Hãy mở 背包 và click vào biểu tượng Ngôi Sao của bất kỳ con cá nào.", "INFO", 5)
                return
            end
            FM.spyEnabled = true
        ...
        createButtonRow(singleCard, "Quét Lại 背包", "Đếm và cập nhật lại 全部 các khối dữ liệu", "🔄 Quét Lại", function()
            if UpdateCrimsonBreamUI then UpdateCrimsonBreamUI() end
            ShowNotification("Quét Hoàn Tất", "已 đồng bộ toàn bộ số liệu 背包 vào Tab Quản Lý Cá!", "SUCCESS", 4)
        end)"Quét Hoàn Tất", "Đã đồng bộ 全部 số liệu 背包 vào Tab 鱼类管理!", "SUCCESS", 4)
        end)
    end
...
do
createCategoryHeader(tabGod, "神灵互动（God Spirit）")
local godCard = createCardGroup(tabGod)

createButtonRow(godCard, "检查 神灵", "检查 符咒 幸运 hiện tại (Vàng / Xanh)", "检查", function()
    local pData = ReplicatedStorage:FindFirstChild("Data") and ReplicatedStorage.Data:FindFirstChild(LocalPlayer.UserId)
    if pData and pData:FindFirstChild("GodSpirit") then
        ...
        ShowNotification("神灵", "无法读取数据 神灵.", "WARN")
    end
end)"状态 神灵", msg, "SUCCESS", 6)
    else
        ShowNotification("神灵", "无法读取数据 神灵.", "WARN")
    end
end)

createToggleRow(godCard, "自动 祈祷", "Tự cầu nguyện nhận bùa khi đứng gần Đền Thần", Config.AutoPrayGodSpirit, function(v) Config.AutoPrayGodSpirit = v end)

createButtonRow(godCard, "飞往 Đền 神灵", "传送 đến 祭坛 神灵 (Battlefield Isle)", "飞往", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root then
        ...
        if spirit then
            ...
            ShowNotification("神灵", "已传送到 祭坛 神灵!", "SUCCESS")
        else
            root.CFrame = CFrame.new(1245.7, 19.3, -133.4)
            ShowNotification("神灵", "已传送到 神殿!", "SUCCESS")
        end
    end
end)"神灵", "已传送 đến 祭坛 神灵!", "SUCCESS")
        else
            root.CFrame = CFrame.new(1245.7, 19.3, -133.4)
            ShowNotification("神灵", "已传送 đến 神殿!", "SUCCESS")
        end
    end
end)

createButtonRow(godCard, "祈祷 Ngay Lập Tức", "Tương tác với 祭坛 神灵 ngay bây giờ", "祈祷", function()
    ...
    if sp then
        ...
        if found then
            ShowNotification("神灵", "已成功祈祷 神灵 成功!", "SUCCESS")
        else
            ShowNotification("神灵", "未找到神灵的交互按钮。", "WARN")
        end
    else
        ShowNotification("神灵", "当前服务器尚未出现神灵。", "WARN")
    end
end)"神灵", "Đã cầu nguyện 神灵 thành công!", "SUCCESS")
        else
            ShowNotification("神灵", "未找到 nút bấm tương tác trên Thần.", "WARN")
        end
    else
        ShowNotification("神灵", "服务器 này hiện chưa xuất hiện 神灵.", "WARN")
    end
end)

createCategoryHeader(tabGod, "自动换服")
local hopCard = createCardGroup(tabGod)
createToggleRow(hopCard, "切换服务器 Tìm 神灵", "Tự động nhảy server 持续 đến khi gặp God Spirit", Config.AutoServerHopGod, function(v)
    Config.AutoServerHopGod = v
    secretBossState.SaveNPCHopState()
end)
createToggleRow(hopCard, "切换服务器 Tìm Maoshan", "Tự động nhảy server 持续 đến khi gặp Maoshan", Config.AutoServerHopMaoshan, function(v)
    Config.AutoServerHopMaoshan = v
    secretBossState.SaveNPCHopState()
end)
createToggleRow(hopCard, "切换服务器 Tìm Taoist", "Tự động nhảy server 持续 đến khi gặp Đạo sĩ Taoist", Config.AutoServerHopTaoist, function(v)
    Config.AutoServerHopTaoist = v
    secretBossState.SaveNPCHopState()
end)
createButtonRow(hopCard, "切换服务器 Ngay", "Chuyển sang một 服务器 随机 khác ngay lập tức", "切换服务器", function() ServerHop() end)
end

do
createCategoryHeader(tabQuests, "票券任务状态")
local questCard = createCardGroup(tabQuests)

createToggleRow(questCard, "自动 Làm 任务票 任务", "Tự động nhận, thực hiện và trả vé nhiệm vụ theo 周期 20p", Config.AutoTicketQuest, function(v)
    Config.AutoTicketQuest = v
    if v then
        ticketQuestState.active = true
        ShowNotification("任务票", "已 bật tự động làm 任务票 任务! Script sẽ quét và thực hiện quest.", "SUCCESS", 6)
    else
        ...
        ShowNotification("任务票", "已 tắt tự động làm 任务票 任务.", "INFO", 4)
    end
    ticketQuestState.ScanAndUpdateStatus()
end)"任务票", "Đã bật tự động làm vé nhiệm vụ! Script sẽ quét và thực hiện quest.", "SUCCESS", 6)
    else
        if ticketQuestState then
            ticketQuestState.active = false
            ticketQuestState.isBusyRoutine = false
...
createDropdownRow(questCard, "Độ Khó 任务", "Chọn độ khó vé nhiệm vụ nhận từ NPC (Mặc định: Hard)", {"Hard", "Easy"}, Config.TicketDifficulty, function(v)
...
createDropdownRow(questCard, "任务模式", "Tự động nhận diện từ game hoặc ép kiểu nhiệm vụ bạn muốn bot làm", questModes, Config.TicketQuestMode, function(v)
...
ticketQuestState.uiStatus = createInfoRow(questCard, "任务 Hiện Tại", ticketQuestState.statusText)
ticketQuestState.uiProgress = createInfoRow(questCard, "Tiến Độ 任务", "0 / 100 (0%)")
ticketQuestState.uiCooldown = createInfoRow(questCard, "冷却 20 分钟", "可以领取任务票了！")

createCategoryHeader(tabQuests, "⚡ Zeng Tianguo技能任务与并行模式")
local zengCard = createCardGroup(tabQuests)

zengTianguoQuestState.uiParallelBadge = createInfoRow(zengCard, "制作 Độ Vận Hành", "已关闭")

createToggleRow(zengCard, "自动 任务 Zeng Tianguo", "Tự động nhận, thực hiện và trả nhiệm vụ nâng cấp 技能 của Zeng Tianguo", Config.AutoZengTianguoQuest, function(v)
    Config.AutoZengTianguoQuest = v
    if v then
        zengTianguoQuestState.active = true
        ShowNotification("Zeng Tianguo", "已 bật tự động làm 任务 Zeng Tianguo (Skill Upgrade)!", "SUCCESS", 5)
    else
        zengTianguoQuestState.active = false
        ShowNotification("Zeng Tianguo", "已 tắt tự động làm 任务 Zeng Tianguo.", "INFO", 4)
    end
    zengTianguoQuestState.DetectActiveQuest()
    zengTianguoQuestState.UpdateUI()
end)"Zeng Tianguo", "Đã bật tự động làm nhiệm vụ Zeng Tianguo (Skill Upgrade)!", "SUCCESS", 5)
    else
        zengTianguoQuestState.active = false
        ShowNotification("Zeng Tianguo", "已 tắt tự động làm 任务 Zeng Tianguo.", "INFO", 4)
    end
...
createToggleRow(zengCard, "自动 Trả Quest Zeng Tianguo", "Tự động bay về NPC trả quest khi hoàn thành chuỗi mục tiêu", Config.ZengTianguoAutoClaim, function(v)
    Config.ZengTianguoAutoClaim = v
end)

zengTianguoQuestState.uiStatus = createInfoRow(zengCard, "任务 Kỹ Năng", zengTianguoQuestState.statusText)
zengTianguoQuestState.uiProgress = createInfoRow(zengCard, "Tiến Độ Kỹ Năng", "0 / 100 (0%)")

createButtonRow(zengCard, "Tìm & 飞往 NPC Zeng Tianguo", "自动 搜索 vị trí NPC Zeng Tianguo (Skill Upgrade) và bay tới đối diện", "飞往 NPC", function()
    local ok = zengTianguoQuestState.TeleportToNPC()
    if ok then
        ShowNotification("Dịch Chuyển", "已 bay đến NPC Zeng Tianguo (Skill Upgrade)!", "SUCCESS")
    else
        ShowNotification("Dịch Chuyển", "未找到 model NPC Zeng Tianguo trong game!", "WARNING")
    end
end)"传送", "已飞到 đến NPC Zeng Tianguo (Skill Upgrade)!", "SUCCESS")
    else
        ShowNotification("传送", "未找到 model NPC Zeng Tianguo trong game!", "WARNING")
    end
end)

createButtonRow(zengCard, "Nhận / Nộp Quest Zeng Tianguo", "Tương tác 快速 với NPC Zeng Tianguo để nhận hoặc nộp 任务 hoàn thành", "Tương Tác", function()
    task.spawn(function()
        ShowNotification("Zeng Tianguo", "正在 tương tác với NPC Zeng Tianguo...", "INFO", 3)
...
end)"Zeng Tianguo", "Đang tương tác với NPC Zeng Tianguo...", "INFO", 3)
        local isDone = zengTianguoQuestState.isCompleted
        zengTianguoQuestState.InteractNPC(isDone)
        task.wait(1.0)
        zengTianguoQuestState.DetectActiveQuest()
...
ticketQuestState.ui100Spot = createInfoRow(spotCard, "Điểm 钓鱼 100 Con (Map 1)", string.format("(%.0f, %.0f, %.0f)", ticketQuestState.spot100Fish.X, ticketQuestState.spot100Fish.Y, ticketQuestState.spot100Fish.Z))
createButtonRow(spotCard, "Lấy Tọa Độ Hiện Tại Làm Điểm 100 Con", "Gán vị trí & hướng nhìn bạn 正在 đứng làm nơi 钓鱼 100 con cá nhẹ", "获取位置", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root then
        ...
        ticketQuestState.SaveSpots()
        ticketQuestState.UpdateUI()
        ShowNotification("位置 Nhiệm Vụ", string.format("已 lưu vị trí & hướng nhìn 钓鱼 100 con: (%.0f, %.0f, %.0f)!", root.Position.X, root.Position.Y, root.Position.Z), "SUCCESS")
    end
end)"任务位置", string.format("已保存 vị trí & hướng nhìn câu 100 con: (%.0f, %.0f, %.0f)!", root.Position.X, root.Position.Y, root.Position.Z), "SUCCESS")
    end
end)
createButtonRow(spotCard, "飞往 Điểm 钓鱼 100 Con", "传送 tức thì đến điểm 钓鱼 100 con 已 cài", "飞往", function()
    ticketQuestState.TeleportTo(ticketQuestState.spot100Fish)
    ShowNotification("Dịch Chuyển", "已 bay đến điểm 钓鱼 100 con!", "SUCCESS")
end)"传送", "已飞到 đến điểm câu 100 con!", "SUCCESS")
end)

ticketQuestState.ui100BaitSpot = createInfoRow(spotCard, "Điểm Tiêu Thụ 100 鱼饵 (Map 1)", string.format("(%.0f, %.0f, %.0f)", ticketQuestState.spot100Bait.X, ticketQuestState.spot100Bait.Y, ticketQuestState.spot100Bait.Z))
createButtonRow(spotCard, "Lấy Tọa Độ Hiện Tại Làm Điểm 100 鱼饵", "Gán vị trí & hướng nhìn bạn 正在 đứng làm nơi 钓鱼 tiêu thụ 100 鱼饵", "获取位置", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root then
        ...
        ticketQuestState.SaveSpots()
        ticketQuestState.UpdateUI()
        ShowNotification("位置 Nhiệm Vụ", string.format("已 lưu vị trí & hướng nhìn 100 鱼饵: (%.0f, %.0f, %.0f)!", root.Position.X, root.Position.Y, root.Position.Z), "SUCCESS")
    end
end)"任务位置", string.format("已保存 vị trí & hướng nhìn 100 mồi: (%.0f, %.0f, %.0f)!", root.Position.X, root.Position.Y, root.Position.Z), "SUCCESS")
    end
end)
createButtonRow(spotCard, "飞往 Điểm 100 鱼饵", "传送 tức thì đến điểm 钓鱼 100 鱼饵 已 cài", "飞往", function()
    ticketQuestState.TeleportTo(ticketQuestState.spot100Bait)
    ShowNotification("Dịch Chuyển", "已 bay đến điểm 100 鱼饵!", "SUCCESS")
end)"传送", "已飞到 đến điểm 100 mồi!", "SUCCESS")
end)

ticketQuestState.ui100SkillSpot = createInfoRow(spotCard, "Điểm 钓鱼 100 Skill (Map 1)", string.format("(%.0f, %.0f, %.0f)", ticketQuestState.spot100Skill.X, ticketQuestState.spot100Skill.Y, ticketQuestState.spot100Skill.Z))
createButtonRow(spotCard, "Lấy Tọa Độ Hiện Tại Làm Điểm 100 Skill", "Gán vị trí & hướng nhìn bạn 正在 đứng làm nơi 钓鱼 100 skill", "获取位置", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root then
        ...
        ticketQuestState.SaveSpots()
        ticketQuestState.UpdateUI()
        ShowNotification("位置 Nhiệm Vụ", string.format("已 lưu vị trí & hướng nhìn 100 skill: (%.0f, %.0f, %.0f)!", root.Position.X, root.Position.Y, root.Position.Z), "SUCCESS")
    end
end)"任务位置", string.format("已保存 vị trí & hướng nhìn 100 skill: (%.0f, %.0f, %.0f)!", root.Position.X, root.Position.Y, root.Position.Z), "SUCCESS")
    end
end)
createButtonRow(spotCard, "飞往 Điểm 100 Skill", "传送 tức thì đến điểm 钓鱼 100 skill 已 cài", "飞往", function()
    ticketQuestState.TeleportTo(ticketQuestState.spot100Skill)
    ShowNotification("Dịch Chuyển", "已 bay đến điểm 100 skill!", "SUCCESS")
end)"传送", "已飞到 đến điểm 100 skill!", "SUCCESS")
end)

ticketQuestState.ui15MSpot = createInfoRow(spotCard, "Điểm 钓鱼 1.5M (Map 9)", string.format("(%.0f, %.0f, %.0f)", ticketQuestState.spot15MFish.X, ticketQuestState.spot15MFish.Y, ticketQuestState.spot15MFish.Z))
createButtonRow(spotCard, "Lấy Tọa Độ Hiện Tại Làm Điểm 1.5M", "Gán vị trí & hướng nhìn bạn 正在 đứng làm nơi 钓鱼 1.5M+", "获取位置", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root then
        ...
        ticketQuestState.SaveSpots()
        ticketQuestState.UpdateUI()
        ShowNotification("位置 Nhiệm Vụ", string.format("已 lưu vị trí & hướng nhìn 钓鱼 1.5M: (%.0f, %.0f, %.0f)!", root.Position.X, root.Position.Y, root.Position.Z), "SUCCESS")
    end
end)"任务位置", string.format("已保存 vị trí & hướng nhìn câu 1.5M: (%.0f, %.0f, %.0f)!", root.Position.X, root.Position.Y, root.Position.Z), "SUCCESS")
    end
end)
createButtonRow(spotCard, "飞往 Điểm 钓鱼 1.5M", "传送 tức thì đến điểm 钓鱼 1.5M+ 已 cài", "飞往", function()
    ticketQuestState.TeleportTo(ticketQuestState.spot15MFish)
    ShowNotification("Dịch Chuyển", "已 bay đến điểm 钓鱼 1.5M!", "SUCCESS")
end)"传送", "已飞到 đến điểm câu 1.5M!", "SUCCESS")
end)

ticketQuestState.uiNPCSpot = createInfoRow(spotCard, "位置 NPC Ticket Quest (Map 1)", string.format("(%.0f, %.0f, %.0f)", ticketQuestState.spotNPC.X, ticketQuestState.spotNPC.Y, ticketQuestState.spotNPC.Z))
createButtonRow(spotCard, "Lấy Tọa Độ Hiện Tại Làm 位置 NPC", "Gán vị trí & hướng nhìn bạn 正在 đứng cạnh NPC Ticket Quest", "获取位置", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root then
        ...
        ticketQuestState.SaveSpots()
        ticketQuestState.UpdateUI()
        ShowNotification("位置 NPC", string.format("已 lưu vị trí & hướng nhìn NPC Ticket Quest: (%.0f, %.0f, %.0f)!", root.Position.X, root.Position.Y, root.Position.Z), "SUCCESS")
    end
end)"位置 NPC", string.format("已保存 vị trí & hướng nhìn NPC Ticket Quest: (%.0f, %.0f, %.0f)!", root.Position.X, root.Position.Y, root.Position.Z), "SUCCESS")
    end
end)
createButtonRow(spotCard, "Tìm & 飞往 NPC Ticket Quest", "自动 quét, xoay góc nhìn và bay thẳng đến NPC Ticket Quest", "飞往 NPC", function()
    local npcModel, focusPos, prompt = ticketQuestState.TeleportToNPC()
    if focusPos then
        ticketQuestState.spotNPC = focusPos
        ticketQuestState.SaveSpots()
        ticketQuestState.UpdateUI()
        ShowNotification("Dịch Chuyển", "已找到, căn góc nhìn chuẩn và bay đến NPC Ticket Quest!", "SUCCESS")
    else
        ticketQuestState.TeleportTo(ticketQuestState.spotNPC)
        ShowNotification("Dịch Chuyển", "已 bay đến tọa độ lưu của NPC Ticket Quest!", "SUCCESS")
    end
end)"传送", "Đã tìm thấy, căn góc nhìn chuẩn và bay đến NPC Ticket Quest!", "SUCCESS")
    else
        ticketQuestState.TeleportTo(ticketQuestState.spotNPC)
        ShowNotification("传送", "已飞到 đến tọa độ lưu của NPC Ticket Quest!", "SUCCESS")
    end
end)

createCategoryHeader(tabQuests, "⚙️ 任务鱼饵与技能设置")
...
createDropdownRow(optionCard, "鱼饵 Cho 任务 100 鱼饵", "Loại mồi bot sẽ mua và dùng khi nhận nv 100 mồi", ticketBaits, Config.TicketBaitChoice, function(v)
    Config.TicketBaitChoice = v
end)

local skillList = {"Chiêu Z", "Chiêu X", "Chiêu C", "Chiêu V"}
createDropdownRow(optionCard, "100 次技能任务使用的技能", "技能 bot dùng sau 3s khóa chiêu rồi cất cần lặp lại", skillList, Config.TicketSkillKey, function(v)
    Config.TicketSkillKey = v
end)

createDropdownRow(optionCard, "100 条鱼快速收竿技能", "技能 mạnh nhất dùng để kết liễu cá Map 1 trong 1 hit", skillList, Config.TicketQuickSkill, function(v)
    Config.TicketQuickSkill = v
end)

createToggleRow(optionCard, "背包满后自动卖鱼（任务票）", "Tự động bán sạch cá khi balo đạt giới hạn để câu tiếp", Config.TicketAutoSellFull, function(v)
    Config.TicketAutoSellFull = v
end)

createToggleRow(optionCard, "Tự Về Home Spot 钓鱼 Farm (Chờ 20p)", "Khi trả xong vé và chờ hồi 20p, tự bay về Home Spot và tự động câu cá/combo", Config.TicketReturnHomeWhenDone, function(v)
    Config.TicketReturnHomeWhenDone = v
    Config.TicketAutoCastAtHome = v
end)

createToggleRow(optionCard, "远程领取和提交任务票", "Đứng yên tại chỗ câu để nhận và nộp vé Hard từ xa (không cần bay về NPC)", Config.TicketRemoteClaim, function(v)
    Config.TicketRemoteClaim = v
end)

createCategoryHeader(tabQuests, "⚡ 快速手动操作")
local manualCard = createCardGroup(tabQuests)

createButtonRow(manualCard, "Nhận 任务票 Hard Ngay", "Tương tác NPC, mở hội thoại và bấm nút Quest để nhận 任务票 mới", "Nhận Hard", function()
    task.spawn(function()
        ShowNotification("任务票", "正在 tương tác NPC nhận 任务票 Hard...", "INFO", 3)
        ticketQuestState.InteractNPC(false)
    end)
end)"任务票", "Đang tương tác NPC nhận vé Hard...", "INFO", 3)
        ticketQuestState.InteractNPC(false)
    end)
end)

createButtonRow(manualCard, "Nộp / Trả 任务票 Hard Ngay", "Tương tác NPC, mở hội thoại và bấm nút *Leave* để trả 任务票 & nhận quà", "Nộp Hard", function()
    task.spawn(function()
        ShowNotification("任务票", "正在 tương tác NPC nộp 任务票 Hard...", "INFO", 3)
        ticketQuestState.InteractNPC(true)
    end)
end)"任务票", "Đang tương tác NPC nộp vé Hard...", "INFO", 3)
        ticketQuestState.InteractNPC(true)
    end)
end)

createButtonRow(manualCard, "Quét Lại Tiến Độ 任务", "Quét ngay lập tức PlayerGui để kiểm tra 任务 và tiến độ hiện tại", "Quét Ngay", function()
    ticketQuestState.ScanAndUpdateStatus()
    ShowNotification("任务票", tostring(ticketQuestState.statusText), "INFO", 5)
end)"任务票", tostring(ticketQuestState.statusText), "INFO", 5)
end)

createButtonRow(manualCard, "Đặt Lại / Bỏ Chặn Hết 任务票 Hôm Nay", "Xóa cờ đánh dấu hết 任务票 hôm nay để bot 尝试 tương tác nhận 任务票 lại", "🔄 Đặt Lại", function()
    ticketQuestState.allQuestsDoneForToday = false
    ticketQuestState.allQuestsDoneDate = nil
    ticketQuestState.allQuestsDoneUtcDate = nil
    ticketQuestState.savedDailyCount = nil
    ...
    ticketQuestState.UpdateUI()
    ShowNotification("任务票", "已 xóa cờ hết 任务票 hôm nay! Bot sẽ 尝试 nhận 任务票 lại.", "SUCCESS", 5)
end)"任务票", "Đã xóa cờ hết vé hôm nay! Bot sẽ thử nhận vé lại.", "SUCCESS", 5)
end)

createCategoryHeader(tabQuests, "签到与每日任务")
local dailyCard = createCardGroup(tabQuests)
createButtonRow(dailyCard, "Nhận Thưởng 任务 Ngày", "Tự kiểm tra và nhận thưởng các quest 已 xong", "Nhận Thưởng", function()
    if Events and Events:FindFirstChild("ClaimQuest") then
        for i = 1, 4 do Events.ClaimQuest:FireServer("Daily", i) end
        ShowNotification("Nhiệm Vụ Ngày", "已领取 thưởng 全部 任务 ngày hoàn thành!", "SUCCESS")
    end
end)"任务 Ngày", "Đã nhận thưởng tất cả nhiệm vụ ngày hoàn thành!", "SUCCESS")
    end
end)
...
createButtonRow(dailyCard, "Nhận Hết Quà 7 Ngày", "Nhận 快速 全部 quà điểm danh 7 ngày cùng lúc", "Nhận Hết", function()
    task.spawn(function()
        if Events and Events:FindFirstChild("DailyReward") then
            ...
            ShowNotification("Điểm Danh", "已领取 trọn bộ quà điểm danh từ ngày 1 - 7!", "SUCCESS")
        end
    end)
end)"Điểm Danh", "Đã nhận trọn bộ quà điểm danh từ ngày 1 - 7!", "SUCCESS")
        end
    end)
end)

createButtonRow(dailyCard, "Nhập Toàn Bộ Mã Code", "自动 nhập 全部 hơn 100 mã giftcode (65KLikes, 41MVisits, PVP, SoTamOrb...)", "Nhập Code", function()
    local codes = {
    ...
    if Events and Events:FindFirstChild("RedeemCode") then
        task.spawn(function()
            ...
            ShowNotification("Giftcode", string.format("已 nạp toàn bộ %d mã code vào game!", #codes), "SUCCESS")
        end)
    end
end)"Giftcode", string.format("Đã nạp 全部 %d mã code vào game!", #codes), "SUCCESS")
        end)
    end
end)
...
createInputRow(dailyCard, "Nhập Mã Code Thủ Công", "Gõ mã giftcode riêng hoặc mã mới ra để nạp ngay", "", function(codeTxt)
    if codeTxt and codeTxt:gsub("%s+", "") ~= "" then
        ...
        if Events and Events:FindFirstChild("RedeemCode") then
            ...
            ShowNotification("Giftcode", "已 发送 mã: " .. cleanCode, "SUCCESS")
        end
    end
end)
end
...
do
createCategoryHeader(tabShop, "制作与购买鱼饵")
local baitCard = createCardGroup(tabShop)

local craftBaits = {"Nameless Bait", "Frost Bait", "Rainbow Bait"}
createDropdownRow(baitCard, "Chọn 鱼饵 Cần 制作", "Loại mồi thần thoại muốn chế tạo", craftBaits, Config.CraftBaitName, function(v) Config.CraftBaitName = v end)
createSliderRow(baitCard, "Số Lượng 制作 每 次", "Số lượng mồi chế trong 1 lượt", 1, 10, Config.CraftAmount, false, "", function(v) Config.CraftAmount = v end)
createToggleRow(baitCard, "自动 制作 鱼饵", "Liên tục chế mồi khi trong kho đủ nguyên liệu (tự mở khóa NL)", Config.AutoCraftBait, function(v)
...
local btnUnlockCurBait = createButtonRow(baitCard, "解锁 Cá 制作 鱼饵 正在 Chọn", "Chỉ mở khóa các con cá làm 材料 cho 鱼饵 正在 chọn ở trên để chế tạo", "🔓 Mở Khóa NL", function()
...
local btnLockCurBait = createButtonRow(baitCard, "锁定 Lại Cá 制作 鱼饵 正在 Chọn", "锁定 保护 lại cá làm 鱼饵 sau khi chế xong để tránh AutoSell bán mất", "🔒 Khóa Lại NL", function()
...
local btnUnlockAllBait = createButtonRow(baitCard, "解锁 NL Cả 3 Loại 鱼饵 Thần Thoại", "解锁 全部 cá làm 鱼饵 Nameless, Frost, Rainbow Bait để chế tạo", "🔓 Mở Cả 3 Mồi", function()
...
local btnLockAllBait = createButtonRow(baitCard, "锁定 Lại NL Cả 3 Loại 鱼饵 Thần Thoại", "锁定 保护 lại 全部 cá làm 鱼饵 của cả 3 loại 鱼饵 sau khi chế xong", "🔒 Khóa Cả 3 Mồi", function()
...
createDropdownRow(baitCard, "Chọn 鱼饵 Cần 购买", "Loại mồi muốn mua từ NPC Ba Chang", buyBaits, Config.BuyBaitName, function(v) Config.BuyBaitName = v end)
createSliderRow(baitCard, "Số Lượng 购买 每 次", "Số lượng mồi mua 每 次 giao dịch", 1, 50, Config.BuyBaitAmount, false, "", function(v) Config.BuyBaitAmount = v end)
createSliderRow(baitCard, "Ngưỡng 购买 自动", "Tự mua khi số mồi trong kho ít hơn mức này", 5, 50, Config.BuyBaitThreshold, false, "", function(v) Config.BuyBaitThreshold = v end)
createToggleRow(baitCard, "自动 购买 鱼饵", "Tự động mua thêm mồi khi sắp hết", Config.AutoBuyBait, function(v) Config.AutoBuyBait = v end)

createCategoryHeader(tabShop, "技能商人（Sage Yijiu）")
...
createDropdownRow(sageCard, "Chọn Kỹ Năng", "技能 muốn học từ NPC Sage Yijiu", sageSkills, chosenSageSkill, function(v) chosenSageSkill = v end)

createButtonRow(sageCard, "飞往 & 购买 Kỹ Năng", "传送 đến Sage Yijiu và mua chiêu thức", "购买 技能", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if root then
        ...
        if Events and Events:FindFirstChild("BuySkill") then
            ...
            ShowNotification("Sage Yijiu", "已 mua 成功 技能: " .. chosenSageSkill, "SUCCESS")
        end
    end
end)"Sage Yijiu", "Đã mua thành công 技能: " .. chosenSageSkill, "SUCCESS")
        end
    end
end)

createCategoryHeader(tabShop, "幸运抽奖（自动）")
local gachaCard = createCardGroup(tabShop)
createDropdownRow(gachaCard, "选择 Gacha 抽奖", "Vòng quay muốn rút thưởng", {"Taiji Banner", "Egoless Banner"}, Config.GachaBanner, function(v) Config.GachaBanner = v end)
createSliderRow(gachaCard, "每次抽奖使用票数", "Số lượng vé dùng cho 每 lượt rút thưởng", 1, 10, Config.GachaPullsPerAction, false, "", function(v) Config.GachaPullsPerAction = v end)
createToggleRow(gachaCard, "Vòng Quay May Mắn (Auto Gacha)", "Tự động rút thưởng 持续 từ banner đã chọn", Config.AutoGacha, function(v) Config.AutoGacha = v end)
end

createCategoryHeader(tabShop, "鱼竿列表（按价格排序）")
...
createButtonRow(rodShopCard, "Làm Mới 状态 Cần", "Quét lại túi đồ để cập nhật danh sách cần 已 có / chưa có", "Làm Mới", function()
    UpdateAllRodShopUI()
    ShowNotification("Shop Cần", "已 cập nhật trạng thái sở hữu cần 钓鱼!", "INFO")
end)"Shop Cần", "Đã cập nhật trạng thái sở hữu cần câu!", "INFO")
end)
...
for _, rod in ipairs(allRods) do
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if isEquipped then
            ShowNotification("Cần 钓鱼", "Bạn 正在 cầm cần " .. rod.name .. " rồi!", "INFO")
        elseif isOwned then
            if Events and Events:FindFirstChild("EquipFishingRod") then
                Events.EquipFishingRod:InvokeServer(rod.name)
                ShowNotification("Trang Bị Cần", "已装备 cần: " .. rod.name, "SUCCESS")
            ...
            if Events and Events:FindFirstChild("BuyFishingRod") then
                ...
                ShowNotification("Shop Cần", "已 发送 yêu cầu mua cần: " .. rod.name .. " (" .. pStr .. ")", "SUCCESS")
            end
        end
...
do
createCategoryHeader(tabTeleports, "传送到岛屿（岛1 - 10）")
...
local infoCurrentMap = createInfoRow(islandCard, "📍 位置 Bạn 正在 Đứng", "Đang nhận diện...")

createButtonRow(islandCard, "📋 Sao Chép Tọa Độ Hiện Tại", "Copy tọa độ đứng hiện tại vào Clipboard để 发送 cho AI nạp đảo mới", "Sao Chép", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if not root then return end
    local pos = root.Position
    ...
    ShowNotification("TỌA ĐỘ HIỆN TẠI", "已 copy: " .. str .. " vào Clipboard!", "SUCCESS", 6)
end)"TỌA ĐỘ HIỆN TẠI", "Đã copy: " .. str .. " vào Clipboard!", "SUCCESS", 6)
end)
...
for _, isl in ipairs(islands) do
    ...
    local btn = Instance.new("TextButton"); btn.Size = UDim2.new(0, 105, 0, 24); btn.Position = UDim2.new(1, -105, 0.5, -12); btn.BackgroundColor3 = Colors.ControlBg; btn.Font = Enum.Font.GothamBold; btn.Text = "前往"; btn.TextColor3 = Colors.PurplePrimary; btn.TextSize = 11; btn.BorderSizePixel = 0; btn.Parent = row
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 4)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if curLocName == isl.name then
            ShowNotification("传送", "Bạn 正在 ở ngay " .. isl.name .. " rồi!", "INFO")
            return
        end
        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
...
do
createCategoryHeader(tabTeleports, "Boss竞技场与秘密区域")
...
for _, br in ipairs(bossRealms) do
    createButtonRow(bossRealmCard, br.name, "传送 đến " .. br.name, "飞往", function()
        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if root then
    ...
    end)"Teleport", "Arrived at " .. br.name .. "!", "SUCCESS")
        end
    end)
end

createCategoryHeader(tabTeleports, "鱼竿商店（Biao Di）")
...
for _, rd in ipairs(rodDealers) do
    createButtonRow(rodDealerCard, rd.name, "Bay trực tiếp đến " .. rd.name, "飞往", function()
        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if root then
            root.CFrame = CFrame.new(rd.pos + Vector3.new(0, 3, 0))
            ShowNotification("Cửa Hàng", "已到达 " .. rd.name .. "!", "SUCCESS")
        end
    end)"Cửa Hàng", "Đã đến " .. rd.name .. "!", "SUCCESS")
        end
    end)
end
...
for _, sr in ipairs(secretRods) do
    createButtonRow(sRodCard, sr.name, "飞往 vị trí lấy cần: " .. sr.name, "飞往", function()
        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if root then
            root.CFrame = CFrame.new(sr.pos + Vector3.new(0, 3, 0))
            ShowNotification("Cần Bí Mật", "已 bay đến " .. sr.name .. "!", "SUCCESS")
        end
    end)"Cần Bí Mật", "已飞到 đến " .. sr.name .. "!", "SUCCESS")
        end
    end)
end
end

createCategoryHeader(tabTeleports, "👥 传送到地图中的玩家")
...
local playerDropdown = createDropdownRow(srvCard, "Chọn 玩家", "Danh sách người chơi đang có mặt trong server", initialPlayerList, selectedPlayerKey, function(v)
    selectedPlayerKey = v
end)
...
createButtonRow(srvCard, "飞往 玩家 已 Chọn", "传送 tức thì đến ngay bên cạnh người chơi 正在 chọn", "🚀 飞往", function()
    ...
    if not targetPlayer or not targetPlayer.Parent then
        ShowNotification("Dịch Chuyển", "请 chọn người chơi hợp lệ!", "WARN")
    ...
    if myRoot and tRoot then
        ...
        ShowNotification("Dịch Chuyển", "已 bay đến người chơi: " .. targetPlayer.DisplayName, "SUCCESS")
        RefreshPlayerDropdown()
    else
        ShowNotification("Dịch Chuyển", "Người chơi này chưa hồi sinh hoặc không có nhân vật!", "WARN")
    end
end)"传送", "请 chọn người chơi hợp lệ!", "WARN")
        RefreshPlayerDropdown()
        return
    end
    ...
    if myRoot and tRoot then
        myRoot.CFrame = tRoot.CFrame + Vector3.new(0, 2, 3)
        ShowNotification("传送", "已飞到 đến người chơi: " .. targetPlayer.DisplayName, "SUCCESS")
        RefreshPlayerDropdown()
    else
        ShowNotification("传送", "Người chơi này chưa hồi sinh hoặc không có nhân vật!", "WARN")
    end
end)

createButtonRow(srvCard, "Làm Mới Danh Sách 玩家", "Cập nhật danh sách người chơi vừa tham gia hoặc rời 服务器", "🔄 Làm Mới", function()
    RefreshPlayerDropdown()
    ShowNotification("Danh Sách", "已 cập nhật danh sách người chơi trong map!", "INFO")
end)"Danh Sách", "Đã cập nhật danh sách người chơi trong map!", "INFO")
end)

local lastTpTarget = nil
createButtonRow(srvCard, "飞往 玩家 Ngẫu Nhiên", "传送 tức thì đến vị trí của một người chơi bất kỳ", "🎲 Ngẫu Nhiên", function()
    local targets = {}
    for _, p in ipairs(Players:GetPlayers()) do
    ...
    if #targets == 0 then
        ShowNotification("Dịch Chuyển", "未找到 người chơi nào khác trong 服务器.", "WARN")
        return
    end
    local pool = {}
    ...
    if root and selected.Character and selected.Character:FindFirstChild("HumanoidRootPart") then
        ...
        ShowNotification("Dịch Chuyển", "已 bay đến người chơi: " .. selected.DisplayName, "SUCCESS")
        RefreshPlayerDropdown()
    end
end)"传送", "未找到 người chơi nào khác trong server.", "WARN")
        return
    end
    local pool = {}
    for _, p in ipairs(targets) do
    ...
    if root and selected.Character and selected.Character:FindFirstChild("HumanoidRootPart") then
        root.CFrame = selected.Character.HumanoidRootPart.CFrame + Vector3.new(0, 2, 3)
        ShowNotification("传送", "已飞到 đến người chơi: " .. selected.DisplayName, "SUCCESS")
        RefreshPlayerDropdown()
    end
end)
...
-- ============================================================
do
createCategoryHeader(tabTeleports, "🧙 传送到任务NPC")
...
for _, npc in ipairs(questNPCList) do
    createButtonRow(
        ...
        function()
            local char = LocalPlayer.Character
            local root = char and char:FindFirstChild("HumanoidRootPart")
            if not root then ShowNotification("错误", "Nhân vật chưa spawn!", "ERROR") return end
            ...
            if model then
                ...
                if npcRoot then
                    ...
                    ShowNotification("✅ Đến " .. npc.name, "已 bay đến NPC " .. npc.name .. "!", "SUCCESS", 4)
                    return
                end
            end
            ShowNotification("❌ Không Tìm Thấy", "NPC '" .. npc.name .. "' không có trong map lúc này. Có thể chưa spawn hoặc 正在 ở đảo khác.", "WARN", 5)
        end
    )"Lỗi", "Nhân vật chưa spawn!", "ERROR") return end
            local model = findNPCModel(npc.name, npc.path)
            if model then
                local npcRoot = model:FindFirstChild("HumanoidRootPart")
                    or model:FindFirstChildWhichIsA("BasePart")
                if npcRoot then
                    root.CFrame = CFrame.new(npcRoot.Position + Vector3.new(0, 3, 3))
                    ShowNotification("✅ Đến " .. npc.name, "已飞到 đến NPC " .. npc.name .. "!", "SUCCESS", 4)
                    return
                end
            end
            ShowNotification("❌ Không Tìm Thấy", "NPC '" .. npc.name .. "' không có trong map lúc này. Có thể chưa spawn hoặc 正在 ở đảo khác.", "WARN", 5)
...
-- ============================================================
createCategoryHeader(tabTeleports, "📜 传送到道士（Taoist & Maoshan）")
local taoistTeleCard = createCardGroup(tabTeleports)

createButtonRow(taoistTeleCard, "📜 飞往 Đạo Sĩ (Taoist)", "传送 tức thì đến NPC Đạo Sĩ (Taoist) nếu có trong 服务器", "飞往", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if not root then ShowNotification("错误", "Nhân vật chưa spawn!", "ERROR") return end
    ...
    if tInst then
        ...
        if pivot then
            ...
            ShowNotification("Đạo Sĩ (Taoist)", "已传送到 vị trí " .. tostring(tName) .. "!", "SUCCESS", 5)
        else
            ShowNotification("Đạo Sĩ (Taoist)", "Không lấy được tọa độ Đạo Sĩ.", "WARN")
        end
    else
        ShowNotification("Đạo Sĩ (Taoist)", "服务器 này hiện chưa có Đạo Sĩ (Taoist)! Hãy bật '切换服务器 Tìm Taoist'.", "WARN", 6)
    end
end)"Lỗi", "Nhân vật chưa spawn!", "ERROR") return end
    local tInst, tName = secretBossState.ScanForTaoistNPC()
    if tInst then
        local pivot = (tInst:IsA("Model") and tInst:GetPivot()) or (tInst:IsA("BasePart") and tInst.CFrame)
        if pivot then
            root.CFrame = pivot + Vector3.new(0, 3, 4)
            ShowNotification("Đạo Sĩ (Taoist)", "已传送 đến vị trí " .. tostring(tName) .. "!", "SUCCESS", 5)
        else
            ShowNotification("Đạo Sĩ (Taoist)", "Không lấy được tọa độ Đạo Sĩ.", "WARN")
        end
    else
...
createButtonRow(taoistTeleCard, "✨ 飞往 Đạo Sĩ Maoshan", "传送 tức thì đến NPC Đạo Sĩ Maoshan nếu có trong 服务器", "飞往", function()
    local char = LocalPlayer.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    if not root then ShowNotification("错误", "Nhân vật chưa spawn!", "ERROR") return end
    ...
    if mInst then
        ...
        if pivot then
            root.CFrame = pivot + Vector3.new(0, 3, 4)
            ShowNotification("Đạo Sĩ Maoshan", "已传送到 vị trí " .. tostring(mName) .. "!", "SUCCESS", 5)
        else
            ShowNotification("Đạo Sĩ Maoshan", "Không lấy được tọa độ Đạo Sĩ Maoshan.", "WARN")
        end
    else
        ShowNotification("Đạo Sĩ Maoshan", "服务器 này hiện chưa có Đạo Sĩ Maoshan! Hãy bật '切换服务器 Tìm Maoshan'.", "WARN", 6)
    end
end)"Lỗi", "Nhân vật chưa spawn!", "ERROR") return end
    local mInst, mName = secretBossState.ScanForMaoshanNPC()
    if mInst then
        ...
        if pivot then
            root.CFrame = pivot + Vector3.new(0, 3, 4)
            ShowNotification("Đạo Sĩ Maoshan", "已传送 đến vị trí " .. tostring(mName) .. "!", "SUCCESS", 5)
        else
            ShowNotification("Đạo Sĩ Maoshan", "Không lấy được tọa độ Đạo Sĩ Maoshan.", "WARN")
        end
    else
...
do
createCategoryHeader(tabVisuals, "ESP透视")
local espCard = createCardGroup(tabVisuals)

createToggleRow(espCard, "ESP 神灵（God Spirit）", "Hiện vị trí 神灵 xuyên 地图", Config.ESP_GodSpirit, function(v) Config.ESP_GodSpirit = v end)
createToggleRow(espCard, "ESP 秘密鱼竿", "Hiện vị trí các cần câu ẩn trên 地图", Config.ESP_SecretRod, function(v) Config.ESP_SecretRod = v end)
createToggleRow(espCard, "ESP 船只", "Hiện vị trí tất cả thuyền xung quanh", Config.ESP_Boats, function(v) Config.ESP_Boats = v end)
...
createToggleRow(espCard, "ESP 道士（Taoist）", "Hiện vị trí NPC hoặc cần Taoist", Config.ESP_Taoist, function(v) Config.ESP_Taoist = v end)
...
createToggleRow(espCard, "ESP 玩家", "Hiện khung & khoảng cách đến người chơi khác", Config.ESP_Players, function(v) Config.ESP_Players = v end)
createToggleRow(espCard, "Ẩn Tên Mặc Định 玩家", "Ẩn 全部 bảng tên, danh hiệu và thanh máu trên đầu của người chơi khác", Config.HideOverheadNames, function(v)
...
createToggleRow(espCard, "Hiện Cân Nặng & Đột Biến Trên Vòng Đỏ", "Hiển thị tên cá, cân nặng (kg) và loại 变异 trực tiếp trên vòng định vị", Config.ShowFishWeightRing, function(v) Config.ShowFishWeightRing = v end)

createCategoryHeader(tabVisuals, "光照与降帧优化")
...
createToggleRow(perfCard, "Tầm Nhìn Xa (Xóa Mờ Map)", "Tắt hiệu ứng làm mờ xa (DepthOfField) & 雾天 mù, nhìn rõ mọi hòn đảo từ xa", Config.ClearFarVision, function(v)
    Config.ClearFarVision = v
    clearVisionState.Apply(v or Config.NoFog)
end)

createToggleRow(perfCard, "Xóa 雾天 Mù & 雨天 暴风", "Xóa sạch 雾天 mù, khói mờ, hạt 雨天 và 雷暴 sét che khuất tầm nhìn", Config.NoFog, function(v)
    Config.NoFog = v
...
createToggleRow(perfCard, "Chống Lóa 天气 (Anti-Glare)", "Tự động kìm hãm ánh sáng khi thời tiết đổi sang 大风 暴风, trời 晴天 chói", Config.FullbrightAntiGlare, function(v)
...
createToggleRow(perfCard, "制作 Độ Giảm Lag (Low GFX)", "Tắt bóng đổ và giảm tải đồ họa giúp game siêu mượt", Config.PerformanceMode, function(v)
...
createButtonRow(perfCard, "解锁 Toàn Bộ Sách Cá (Index)", "解锁 khám phá đủ 109 loài cá và vật phẩm trong Index", "Mở Khóa Index", function()
    local count = 0
    local pData = ReplicatedStorage:FindFirstChild("Data") and ReplicatedStorage.Data:FindFirstChild(LocalPlayer.UserId)
    ...
    ShowNotification("解锁 Index", string.format("已解锁 %d loài cá trong Sách Cá Index!", count > 0 and count or 109), "SUCCESS")
end)"Mở Khóa Index", string.format("已解锁 %d loài cá trong Sách Cá Index!", count > 0 and count or 109), "SUCCESS")
end)
end

do
createCategoryHeader(tabPlayer, "📜 技能数据导出")
local exportSkillCard = createCardGroup(tabPlayer)
local infoSkillCount = createInfoRow(exportSkillCard, "Kỹ Năng 已 Quét", "Chưa quét dữ liệu")

createButtonRow(exportSkillCard, "Quét Kỹ Năng 正在 Sở Hữu (Chỉ Của Bạn)", "Chỉ quét các 技能 bạn thực sự sở hữu trong túi đồ & phím Z,X,C,V", "👤 Skill Của Bạn", function()
    ExportAllPlayerSkills(infoSkillCount, true)
end)

createButtonRow(exportSkillCard, "Quét Bách Khoa Toàn Bộ Kỹ Năng Game", "Quét 全部 từ điển 技能 có trong game (Codex/Shop/全部)", "📚 Toàn Bộ Game", function()
...
createCategoryHeader(tabPlayer, "角色移动")
local moveCard = createCardGroup(tabPlayer)

createToggleRow(moveCard, "Tăng Tốc Độ Chạy (Speed)", "Chạy 快速 hơn tốc độ mặc định", Config.WalkSpeedEnabled, function(v)
...
createToggleRow(moveCard, "Nhảy Vô Hạn (Infinite Jump)", "Nhảy 持续 trên không trung không giới hạn", Config.InfiniteJump, function(v) Config.InfiniteJump = v end)
...
createToggleRow(moveCard, "酸雨护盾（Acid Shield）", "Tạo sàn nổi kháng sát thương độc/axit tại Đảo Fallout", Config.AcidWaterShield, function(v) Config.AcidWaterShield = v end)
...
createCategoryHeader(tabPlayer, "防掉线与稳定")
local stabCard = createCardGroup(tabPlayer)
createToggleRow(stabCard, "Chống Văng Game (Anti-AFK)", "Chống bị Roblox kick sau 20 分钟 treo máy", Config.AntiAFK, function(v) Config.AntiAFK = v end)
createToggleRow(stabCard, "自动 Kết Nối Lại", "Tự động vào lại server nếu bị mất kết nối", Config.AutoRejoin, function(v) Config.AutoRejoin = v end)

createCategoryHeader(tabPlayer, "🎭 外观伪装")
...
ticketInputRow = createInputRow(spoofCard, "Ảo Hoá Vé Nhiệm Vụ", "Nhập số vé ảo mong muốn và ấn Enter (Tự lưu vĩnh viễn vào máy)", (visualSpoofState.fakeTicket and visualSpoofState.fakeTicket > 0) and tostring(visualSpoofState.fakeTicket) or "", function(txt)
    ...
    if num > 0 then
        ShowNotification("Ảo Hoá 任务票", "已 ảo hoá 成功: " .. FormatWithSpaces(num) .. " 任务票 (已保存 máy)!", "SUCCESS", 5)
    else
        ShowNotification("Ảo Hoá 任务票", "已 hủy ảo hoá 任务票! Trở về số lượng thực tế.", "INFO", 5)
    end
...
gemsInputRow = createInputRow(spoofCard, "Ảo Hoá Gems / Đá Quý", "Nhập số Gems ảo mong muốn và ấn Enter (Tự lưu vĩnh viễn vào máy)", (visualSpoofState.fakeGems and visualSpoofState.fakeGems > 0) and tostring(visualSpoofState.fakeGems) or "", function(txt)
    ...
    visualSpoofState.Apply()
    if num > 0 then
        ShowNotification("Ảo Hoá Gems", "已 ảo hoá 成功: " .. FormatWithSpaces(num) .. " Gems (已保存 máy)!", "SUCCESS", 5)
    else
        ShowNotification("Ảo Hoá Gems", "已 hủy ảo hoá Gems! Trở về số lượng thực tế.", "INFO", 5)
    end
end, "ao hoa gems da quy", "Nhập số Gems... (VD: 999999)")

createButtonRow(spoofCard, "Khôi Phục Số Thật (Reset)", "Tắt 全部 ảo hoá và khôi phục hiển thị số lượng thực tế", "🔄 Khôi Phục Thật", function()
    ...
    ShowNotification("Ảo Hoá", "已 xóa toàn bộ số ảo và khôi phục số lượng thật!", "SUCCESS", 5)
end)"Ảo Hoá", "Đã xóa 全部 số ảo và khôi phục số lượng thật!", "SUCCESS", 5)
end)
end

do
    createCategoryHeader(tabProfiles, "配置管理")
    ...
    createInfoRow(profCard, "自动 Lưu Thiết Yếu (Máy)", "Tự động lưu Skill, Combo, Tiện ích")

    createButtonRow(profCard, "Lưu Thiết Yếu Vào Máy", "Lưu ngay 全部 Skill, Combo, Tiện ích hiện tại vào file máy", "💾 Lưu Ngay", function()
        if Config._saveEssential then
            Config._saveEssential()
            ShowNotification("Lưu Thiết Yếu", "已 lưu cài đặt thiết yếu vào máy 成功!", "SUCCESS")
        end
    end)"Lưu Thiết Yếu", "已保存 cài đặt thiết yếu vào máy thành công!", "SUCCESS")
        end
    end)

    createButtonRow(profCard, "Nạp Lại Thiết Yếu Từ Máy", "Đọc và cập nhật lại 配置 thiết yếu 已 lưu từ file máy", "🔄 Nạp Lại", function()
        if Config._loadEssential then
            local ok = Config._loadEssential()
            if ok then
                ShowNotification("Nạp Thiết Yếu", "已 nạp cài đặt thiết yếu từ máy 成功!", "SUCCESS")
            else
                ShowNotification("Nạp Thiết Yếu", "Chưa có file 配置 thiết yếu nào trên máy!", "WARN")
            end
        end
    end)"加载核心设置", "Đã nạp cài đặt thiết yếu từ máy thành công!", "SUCCESS")
            else
                ShowNotification("加载核心设置", "暂无 file 配置 thiết yếu nào trên máy!", "WARN")
            end
        end
    end)
    ...
    local configDropdown = createDropdownRow(profCard, "Chọn Cấu Hình 已 Lưu", "Danh sách 全部 配置 riêng của tài khoản " .. curAccName, #initialConfigs > 0 and initialConfigs or {"(Chưa có cấu hình)"}, function(val)
    ...
    createButtonRow(profCard, "Lưu Cấu Hình Mới", "Lưu 全部 cài đặt hiện tại thành một file 配置 mới", "💾 保存配置", function()
        local targetName = newConfigInputName:gsub("^%s+", ""):gsub("%s+$", "")
        if #targetName == 0 then
            ShowNotification("Lưu Config", "请 nhập tên 配置 mới vào ô phía trên!", "WARN")
            return
        end
        local ok, res = SaveAccountConfig(targetName)
        if ok then
            ShowNotification("Lưu Config", "已 lưu 配置 [" .. res .. "] cho tài khoản " .. curAccName .. "!", "SUCCESS")
            RefreshProfileDropdown(res)
        else
            ShowNotification("Lưu Thất Bại", tostring(res), "ERROR")
        end
    end)"保存配置", "请 nhập tên 配置 mới vào ô phía trên!", "WARN")
            return
        end
        local ok, res = SaveAccountConfig(targetName)
        if ok then
            ShowNotification("保存配置", "已保存 配置 [" .. res .. "] cho tài khoản " .. curAccName .. "!", "SUCCESS")
            RefreshProfileDropdown(res)
        else
            ShowNotification("Lưu Thất Bại", tostring(res), "ERROR")
        end
    ...
    createButtonRow(profCard, "Áp Dụng Cấu Hình (Load)", "Tải và đồng bộ hóa 全部 cài đặt từ 配置 已 chọn", "🚀 加载配置", function()
        local curSel = configDropdown.Get() or selectedConfigName
        if not curSel or curSel == "" or curSel == "(Chưa có cấu hình)" then
            ShowNotification("Nạp Config", "Tài khoản " .. curAccName .. " chưa chọn hoặc chưa có 配置 nào!", "WARN")
            return
        end
        local ok, res = LoadAccountConfig(curSel)
        if ok then
            ShowNotification("Nạp Config", "已 nạp và đồng bộ giao diện 配置 [" .. res .. "]!", "SUCCESS")
        else
            ShowNotification("Nạp Thất Bại", tostring(res), "ERROR")
        end
    end)"加载配置", "Tài khoản " .. curAccName .. " chưa chọn hoặc chưa có 配置 nào!", "WARN")
            return
        end
        local ok, res = LoadAccountConfig(curSel)
        if ok then
            ShowNotification("加载配置", "已 nạp và đồng bộ giao diện 配置 [" .. res .. "]!", "SUCCESS")
        else
            ShowNotification("Nạp Thất Bại", tostring(res), "ERROR")
        end
    end)

    createButtonRow(profCard, "Làm Mới Danh Sách", "Quét lại 全部 các file 配置 hiện có của tài khoản", "🔄 Làm Mới", function()
        RefreshProfileDropdown()
        ShowNotification("Danh Sách Config", "已 cập nhật lại danh sách 配置 của " .. curAccName .. "!", "INFO")
    end)"Danh Sách Config", "Đã cập nhật lại danh sách 配置 của " .. curAccName .. "!", "INFO")
    end)

    createButtonRow(profCard, "Xóa Cấu Hình 已 Chọn", "Xóa vĩnh viễn file 配置 正在 được chọn khỏi máy", "🗑️ 删除配置", function()
        local curSel = configDropdown.Get() or selectedConfigName
        if not curSel or curSel == "" or curSel == "(Chưa có cấu hình)" then
            ShowNotification("Xóa Config", "Chưa chọn file 配置 hợp lệ để xóa!", "WARN")
            return
        end
        local ok, res = DeleteAccountConfig(curSel)
        if ok then
            ShowNotification("Xóa Config", "已 xóa 配置 [" .. res .. "] 成功!", "SUCCESS")
            RefreshProfileDropdown()
        else
            ShowNotification("Xóa Thất Bại", tostring(res), "ERROR")
        end
    end)"删除配置", "Chưa chọn file 配置 hợp lệ để xóa!", "WARN")
            return
        end
        local ok, res = DeleteAccountConfig(curSel)
        if ok then
            ShowNotification("删除配置", "已 xóa 配置 [" .. res .. "] 成功!", "SUCCESS")
            RefreshProfileDropdown()
        else
            ShowNotification("Xóa Thất Bại", tostring(res), "ERROR")
        end
    ...
    createButtonRow(profCard, "Xóa Bộ Nhớ Cấu Hình Tự Lưu (Reset Cache Máy)", "Xóa file essential_config.json lưu ngầm trên máy để xóa triệt để cài đặt cũ", "⚠️ Xóa Cache Máy", function()
        ...
        ShowNotification("Reset Cache", "已 xóa sạch file cache máy và đặt lại combo về [Z, X, V]!", "SUCCESS", 6)
    end)"Reset Cache", "Đã xóa sạch file cache máy và đặt lại combo về [Z, X, V]!", "SUCCESS", 6)
    end)
end

do
createCategoryHeader(tabProfiles, "👑 优先级管理")
...
PriorityManager.uiStatusRow = createInfoRow(priCard, "Tác Vụ 正在 Thực Thi", "💤 Đang Chờ (Idle)")
...
local dropPreset = createDropdownRow(priCard, "优先级模板", "Chọn 快速 bộ ưu tiên phổ biến hoặc tự do xếp hạng bên dưới", priorityPresetNames, Config.PriorityPreset or priorityPresetNames[1], function(v)
...
dropSecretBoss = createDropdownRow(priCard, "优先：🎯 狩猎 Secret Boss", "Xếp hạng ưu tiên cho Boss 狩猎 (Chat Sniper & Thời Tiết)", rankOptions, numberToRank(Config.Priority_SecretBoss), function(v)
    Config.Priority_SecretBoss = rankToNumber(v)
    onCustomRankChange()
end)

dropTicketQuest = createDropdownRow(priCard, "优先：📜 完成任务票", "Xếp hạng ưu tiên cho Tự Động Làm Vé 任务 20p", rankOptions, numberToRank(Config.Priority_TicketQuest), function(v)
    Config.Priority_TicketQuest = rankToNumber(v)
    onCustomRankChange()
end)

dropGodSpirit = createDropdownRow(priCard, "优先：⛩️ 供奉神灵", "Xếp hạng ưu tiên cho Tự Động 祈祷 / 供奉 Thần", rankOptions, numberToRank(Config.Priority_GodSpirit), function(v)
    Config.Priority_GodSpirit = rankToNumber(v)
    onCustomRankChange()
end)

dropTrainSkill = createDropdownRow(priCard, "优先：⚔️ 自动练技能", "Xếp hạng ưu tiên cho Fast Cancel luyện cấp 技能", rankOptions, numberToRank(Config.Priority_TrainSkill), function(v)
    Config.Priority_TrainSkill = rankToNumber(v)
    onCustomRankChange()
end)

dropNormalFarm = createDropdownRow(priCard, "优先：🎣 自动挂机刷鱼", "Xếp hạng ưu tiên cho Câu 普通 và Farm tại Home Spot", rankOptions, numberToRank(Config.Priority_NormalFarm), function(v)
...
do
createCategoryHeader(tabProfiles, "📢 Discord Webhook远程通知")
...
createToggleRow(hookCard, "Boss 捕获通知", "Gửi tin nhắn khi câu trúng hoặc bắt thành công Boss / Cá Thần Thoại", Config.WebhookNotifyBoss, function(v)
    Config.WebhookNotifyBoss = v
    SaveNotificationsConfig()
end)

createToggleRow(hookCard, "通知 Đạo Sĩ (Taoist & Maoshan)", "Gửi tin nhắn JobId server khi phát hiện Đạo Sĩ (Taoist) hoặc Mao Sơn (Maoshan)", Config.WebhookNotifyNPC, function(v)
    Config.WebhookNotifyNPC = v
    SaveNotificationsConfig()
end)

createToggleRow(hookCard, "报告 Định Kỳ", "Gửi bảng tổng kết thời gian treo máy, số cá, tiền, boss, thời tiết về Discord", Config.WebhookHourlyStats, function(v)
    Config.WebhookHourlyStats = v
    SaveNotificationsConfig()
end)

createSliderRow(hookCard, "定期报告频率", "Khoảng thời gian tự động gửi báo cáo tiến độ về Discord (分钟)", 5, 120, Config.WebhookStatsInterval or 30, false, " Phút", function(v)
    Config.WebhookStatsInterval = v
    SaveNotificationsConfig()
end)

createToggleRow(hookCard, "任务票完成通知", "Gửi embed đầy đủ về Discord khi nộp vé Hard thành công (số lượt NV, vé, gems)", Config.WebhookNotifyTicketQuest == nil and true or Config.WebhookNotifyTicketQuest, function(v)
    Config.WebhookNotifyTicketQuest = v
    SaveNotificationsConfig()
end)

createButtonRow(hookCard, "发送 报告 Toàn Diện Ngay", "发送 embed 已满 đủ: cá, tiền, gems, 任务票, essence, thời tiết, boss, ba lô, job ID về Discord ngay lập tức", "📊 Gửi Ngay", function()
    if not Config.WebhookUrl or Config.WebhookUrl == "" then
        ShowNotification("Webhook", "请 nhập Webhook URL trước!", "WARN")
        return
    end
    ...
    ShowNotification("Webhook", "正在 tổng hợp báo cáo và 发送 về Discord...", "INFO", 3)
    task.spawn(function()
        local ok = pcall(SendServerReportWebhook, "📊 BÁO CÁO TOÀN DIỆN (YÊU CẦU THỤ CÔNG)")
        if ok then
            ShowNotification("Webhook", "已 发送 báo cáo toàn diện! 检查 kênh Discord của bạn.", "SUCCESS", 5)
        else
            ShowNotification("Webhook", "发送 失败! 检查 lại URL Webhook.", "ERROR")
        end
    end)
end)"Webhook", "请先输入 Webhook URL！", "WARN")
        return
    end
    if not Config.WebhookEnabled then
        Config.WebhookEnabled = true
    ...
    task.spawn(function()
        ...
        if ok then
            ...
            ShowNotification("Webhook", "发送失败！请检查 Webhook URL。", "ERROR")
        end
    end)
end)

createButtonRow(hookCard, "检查 Webhook (Test 快速)", "发送 尝试 1 tin nhắn 连接 đơn giản về Discord ngay lập tức", "Gửi Test", function()
    if not Config.WebhookUrl or Config.WebhookUrl == "" then
        ShowNotification("Webhook", "请 nhập Webhook URL trước!", "WARN")
        return
    end
    ...
    ShowNotification("Webhook", "正在 发送 tin nhắn test...", "INFO")
    task.spawn(function()
        ...
        if ok then
            ShowNotification("Webhook", "已 发送 test! 检查 kênh Discord ngay.", "SUCCESS")
        else
            ShowNotification("Webhook", "发送 失败! 检查 lại URL Webhook.", "ERROR")
        end
    end)
...
createCategoryHeader(tabProfiles, "📱 Telegram机器人（手机通知）")
...
createToggleRow(teleCard, "Boss 捕获通知", "Gửi tin nhắn Telegram khi câu trúng hoặc bắt thành công Boss", Config.TelegramNotifyBoss, function(v)
    Config.TelegramNotifyBoss = v
    SaveNotificationsConfig()
end)

createToggleRow(teleCard, "通知 Đạo Sĩ (Taoist & Maoshan)", "Gửi tin nhắn Telegram kèm JobId khi phát hiện Đạo Sĩ", Config.TelegramNotifyNPC, function(v)
    Config.TelegramNotifyNPC = v
    SaveNotificationsConfig()
end)

createButtonRow(teleCard, "检查 Telegram (Test)", "发送 尝试 1 tin nhắn test đến Telegram của bạn ngay lập tức", "Gửi Test", function()
    if not Config.TelegramBotToken or Config.TelegramBotToken == "" or not Config.TelegramChatId or Config.TelegramChatId == "" then
        ShowNotification("Telegram", "请 nhập Bot Token và Chat ID trước!", "WARN")
        return
    end
    ShowNotification("Telegram", "正在 发送 tin nhắn test đến Telegram...", "INFO")
    task.spawn(function()
        local ok = pcall(function()
        ...
        if ok then
            ShowNotification("Telegram", "已 发送 lệnh test đến Telegram!", "SUCCESS")
        else
            ShowNotification("Telegram", "发送 失败! 检查 lại Token & Chat ID.", "ERROR")
        end
    end)
end)"Telegram", "请 nhập Bot Token và Chat ID trước!", "WARN")
        return
    end
    ShowNotification("Telegram", "正在 发送 tin nhắn test đến Telegram...", "INFO")
    task.spawn(function()
...
createCategoryHeader(tabProfiles, "🔔 ntfy推送（天气与服务器通知）")
...
createToggleRow(ntfyCard, "天气变化通知（ntfy）", "Gửi thông báo ngay khi thời tiết server đổi (雨天, 暴风, 雾天 Mù, 雪天...)", Config.NtfyAlertWeatherChange, function(v)
    Config.NtfyAlertWeatherChange = v
    SaveNotificationsConfig()
end)

createToggleRow(ntfyCard, "天气服务器搜索通知（ntfy）", "Gửi thông báo khi Weather Hop tìm được server có thời tiết mục tiêu", Config.NtfyAlertWeatherHop, function(v)
    Config.NtfyAlertWeatherHop = v
    SaveNotificationsConfig()
end)

createToggleRow(ntfyCard, "Boss 与 NPC 通知（ntfy）", "Gửi thông báo khi phát hiện Secret Boss hoặc Đạo Sĩ (Taoist & Maoshan)", Config.NtfyNotifyBoss, function(v)
    Config.NtfyNotifyBoss = v
    SaveNotificationsConfig()
end)

createButtonRow(ntfyCard, "检查 ntfy (Test)", "发送 尝试 1 thông báo đẩy mẫu về app ntfy trên điện thoại ngay lập tức", "Gửi Test", function()
    local clean = tostring(Config.NtfyTopic or ""):gsub("%s+", "")
    if clean == "" then
        ShowNotification("ntfy", "请 nhập ntfy Topic trước!", "WARN")
        return
    end
    ...
    ShowNotification("ntfy", "正在 发送 thông báo test đến kênh: " .. clean .. "...", "INFO", 4)
    task.spawn(function()
        ...
        ShowNotification("ntfy", "已 发送 thông báo test! Hãy kiểm tra điện thoại của bạn.", "SUCCESS", 5)
    end)
end)"ntfy", "请 nhập ntfy Topic trước!", "WARN")
        return
    end
    -- Tự động bật ntfy nếu người dùng chưa bật để test được ngay
    if not Config.NtfyEnabled then
...
createCategoryHeader(tabProfiles, "其他选项")
local credCard = createCardGroup(tabProfiles)
createButtonRow(credCard, "🔴 Diệt Toàn Bộ Script (Kill Script)", "Ngắt 连接 mọi vòng lặp, xóa sạch giao diện và giải phóng bộ nhớ", "KILL SCRIPT", function()
    ShowNotification("Diệt Script", "正在 ngắt 连接 và đóng script hoàn toàn...", "WARN", 2)
    task.wait(0.2)
    UnloadScript()
end)"关闭脚本", "正在断开连接并完全关闭脚本……", "WARN", 2)
    task.wait(0.2)
    UnloadScript()
end)
end
...
local function initExperimentalTab()
    ...
    createCategoryHeader(tabExperimental, "🎯 自动重置与锁定特性")
    ...
    local infoTraitRerolls = createInfoRow(traitCard, "任务票 Reroll Hiện Có", "Đang tải...")
    ...
    local function RunAutoRerollTrait()
        ...
        task.spawn(function()
            local pData = ReplicatedStorage:FindFirstChild("Data") and ReplicatedStorage.Data:FindFirstChild(LocalPlayer.UserId)
            ShowNotification("Reroll Trait", "开始 tự động Reroll săn Trait: " .. tostring(Config.TargetTraitName), "INFO", 4)
            ...
            while isAutoRerolling and Config.AutoRerollTrait do
                ...
                if currentRerolls <= 0 then
                    ShowNotification("Hết 任务票", "已用完 任务票 Reroll Trait!", "WARN", 5)
                ...
                if Events and Events:FindFirstChild("RerollTrait") then
                    ...
                    if isHit then
                        ShowNotification("TRÚNG TRAIT!", string.format("已 roll trúng [%s]! 自动 khóa 保护 ngay lập tức.", Config.TargetTraitName), "SUCCESS", 8)
    ...
    createToggleRow(traitCard, "自动 Reroll Đến Khi Trúng", "Tự động roll 持续 và khóa lại khi ra đúng Trait mục tiêu", Config.AutoRerollTrait, function(v)
    ...
    createButtonRow(traitCard, "Reroll 1 次 Thủ Công", "Thực hiện roll trait 1 次 ngay lập tức", "Reroll 1 次", function()
        if Events and Events:FindFirstChild("RerollTrait") then
            local res = Events.RerollTrait:InvokeServer()
            UpdateTraitRerollInfo()
            ShowNotification("Reroll Trait", "Kết quả roll: " .. tostring(res or "已 roll 成功"), "INFO", 4)
        end
    end)"Reroll Trait", "Kết quả roll: " .. tostring(res or "Đã roll thành công"), "INFO", 4)
        end
    end)

    createButtonRow(traitCard, "锁定 / 解锁 Trait 正在 Chọn", "Chuyển đổi trạng thái khóa 保护 cho Trait 正在 chọn", "Khóa / Mở", function()
        if Events and Events:FindFirstChild("LockTrait") then
            Events.LockTrait:FireServer(Config.TargetTraitName)
            ShowNotification("锁定 Trait", "已 发送 lệnh đổi trạng thái khóa cho: " .. tostring(Config.TargetTraitName), "SUCCESS", 3)
        end
    end)"Khóa Trait", "Đã gửi lệnh đổi trạng thái khóa cho: " .. tostring(Config.TargetTraitName), "SUCCESS", 3)
        end
    end)
end
initExperimentalTab()
...
local function ProtectInventoryItem(item, showNotify)
    ...
    if shouldProtect and Events and Events:FindFirstChild("FavoriteItem") then
        Events.FavoriteItem:FireServer(item)
        if showNotify then
            ShowNotification("锁定 Cá", string.format("已 tự động KHÓA 保护 [%s] (%s)!", rawName, reason), "SUCCESS", 5)
        end
        return true
    end
    return false
                    ...
                    if not isTrainingBusy then
                        ...
                        task.spawn(function()
                            ...
                            if Config.TrainCurrentCount >= Config.TrainTargetCount then
                                ...
                                ShowNotification("Luyện 技能 Hoàn Tất", string.format("已 luyện đủ %d/%d 次 cho chiêu %s!", Config.TrainCurrentCount, Config.TrainTargetCount, cleanKey), "SUCCESS", 7)
                                pcall(function()
                                    local c = LocalPlayer.Character
        ...
        if isRunning and (Config.AutoServerHopTaoist or Config.AutoServerHopGod or Config.AutoServerHopMaoshan) then
            local hopOk, hopErr = pcall(function()
                ...
                if #foundTargets > 0 then
                    ...
                    ShowNotification("ĐÃ TÌM THẤY!", "已 phát hiện " .. foundStr .. " trong 服务器 này! 已 dừng đổi 服务器.", "SUCCESS", 8)
                ...
                ShowNotification("切换服务器 Tìm NPC", "服务器 này không có " .. targetListStr .. ". 正在 đổi 服务器 tiếp theo...", "WARN", 4)
            ...
            if not inst.Parent or not data.part or not data.part.Parent or not isEnabled then
                ...
                elseif cat == "Boss" then
                    ...
                    if fish and fish.Parent then
                        ...
                        data.subLbl.Text = "⚔️ Enzo召唤与战斗区域"
        ...
        if shouldShow then
            ...
            if Config.ShowFishWeightRing then
                ...
                elseif isPulling then
                    fishRingText.Text = "⚡ 正在拉鱼"
                    fishRingText.Visible = true
                elseif isHooked then
                    fishRingText.Text = "🎣 正在抛竿"
                    fishRingText.Visible = true
                else
                    fishRingText.Visible = false
                end
...
ShowNotification("VIỆT HOÁ V1.4", "Heavyweight Fishing 已 cập nhật: 自动寻找天气服务器, Totem 天气 & Webhook!", "SUCCESS", 6)
