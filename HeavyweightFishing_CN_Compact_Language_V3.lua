...
brandTitle.Text = "钓鱼 PRO"; brandTitle.TextColor3 = Colors.PurplePrimary
...
local footerBrand = Instance.new("TextLabel"); footerBrand.Name = "FooterBrand"; footerBrand.Size = UDim2.new(0, 260, 1, 0); footerBrand.Position = UDim2.new(0, 12, 0, 0); footerBrand.BackgroundTransparency = 1; footerBrand.Font = Enum.Font.Gotham; footerBrand.Text = "Heavyweight Fishing | 中文版"; footerBrand.TextColor3 = Colors.TextMuted; footerBrand.TextSize = 10; footerBrand.TextXAlignment = Enum.TextXAlignment.Left; footerBrand.Parent = footerBar
local footerKey = Instance.new("TextLabel"); footerKey.Name = "FooterKey"; footerKey.Size = UDim2.new(0, 280, 1, 0); footerKey.Position = UDim2.new(1, -292, 0, 0); footerKey.BackgroundTransparency = 1; footerKey.Font = Enum.Font.Gotham; footerKey.Text = "[R-CTRL] 菜单 | [END] 停止脚本"; footerKey.TextColor3 = Colors.TextMuted; footerKey.TextSize = 10; footerKey.TextXAlignment = Enum.TextXAlignment.Right; footerKey.Parent = footerBar
...
-- ============================================================
-- SAFE LANGUAGE SWITCH (placed at the end so it cannot block UI startup)
-- ============================================================
pcall(function()
    local languageButton = Instance.new("TextButton")
    languageButton.Name = "LanguageButton"
    languageButton.Size = UDim2.new(0, 58, 0, 24)
    languageButton.Position = UDim2.new(1, -166, 0, 7)
    languageButton.BackgroundColor3 = Colors.ControlBg
    languageButton.TextColor3 = Colors.PurplePrimary
    languageButton.Font = Enum.Font.GothamBold
    languageButton.TextSize = 10
    languageButton.Text = "中文"
    languageButton.BorderSizePixel = 0
    languageButton.ZIndex = 100
    languageButton.Parent = titleBar
    Instance.new("UICorner", languageButton).CornerRadius = UDim.new(0, 4)

    local languages = {"zh", "en", "vi"}
    local langNames = {zh = "中文", en = "EN", vi = "VI"}
    local langIndex = 1

    local tabTexts = {
        {"Câu Cá", "钓鱼", "Fishing"},
        {"Săn Boss", "Boss 狩猎", "Boss Hunt"},
        {"Quản Lý Cá", "鱼类管理", "Fish Manager"},
        {"Thần Linh", "神灵", "God Spirit"},
        {"Nhiệm Vụ", "任务", "Quests"},
        {"Shop & Chế Mồi", "商店 & 鱼饵", "Shop & Bait"},
        {"Dịch Chuyển", "传送", "Teleport"},
        {"ESP & Đồ Hoạ", "ESP & 画质", "ESP & Visuals"},
        {"Nhân Vật", "角色", "Player"},
        {"Cài Đặt", "设置", "Settings"},
        {"Thử Nghiệm", "实验", "Experimental"},
    }

    local function applySafeLanguage(code)
        for _, item in ipairs(tabTexts) do
            local source, zh, en = item[1], item[2], item[3]
            local text = zh
            if code == "en" then text = en elseif code == "vi" then text = source end
            local btn = tabButtons[source] or tabButtons[zh] or tabButtons[en]
            if btn then btn.Text = text end
        end
        local brand = code == "zh" and "钓鱼 PRO" or (code == "en" and "FISHING PRO" or "CÂU CÁ PRO")
        local subtitle = code == "zh" and "HEAVYWEIGHT FISHING | 中文版" or (code == "en" and "HEAVYWEIGHT FISHING | EN" or "HEAVYWEIGHT FISHING | BẢN VIỆT HOÁ")
        local footerA = code == "zh" and "Heavyweight Fishing | 中文版" or (code == "en" and "Heavyweight Fishing | English" or "Heavyweight Fishing | Việt Hoá V1.1")
        local footerB = code == "zh" and "[R-CTRL] 菜单 | [END] 停止脚本" or (code == "en" and "[R-CTRL] Menu | [END] Stop Script" or "[R-CTRL] Menu | [END] Tắt Script")
        local b = titleBar:FindFirstChild("BrandTitle")
        local s = titleBar:FindFirstChild("GameSubtitle")
        if b then b.Text = brand end
        if s then s.Text = subtitle end
        if footerBar then
            local fa = footerBar:FindFirstChild("FooterBrand")
            local fb = footerBar:FindFirstChild("FooterKey")
            if fa then fa.Text = footerA end
            if fb then fb.Text = footerB end
        end
    end

    languageButton.MouseButton1Click:Connect(function()
        langIndex = langIndex % #languages + 1
        local code = languages[langIndex]
        applySafeLanguage(code)
        languageButton.Text = langNames[code]
    end)
end)
