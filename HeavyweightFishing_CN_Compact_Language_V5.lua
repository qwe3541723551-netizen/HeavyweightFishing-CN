...
-- V4: 禁用左上角悬浮头像，避免卡住屏幕；保留对象以兼容原关闭/最小化逻辑
pcall(function()
    if floatingAvatar then
        floatingAvatar.Visible = false
        floatingAvatar.Active = false
        floatingAvatar.Size = UDim2.new(0, 0, 0, 0)
    end
end)
...
ToggleUiVisibility = function()
    ...
    if floatingAvatar then floatingAvatar.Visible = false end
...
-- ============================================================
-- 中文界面增强层 V2：只处理本脚本 ScreenGui 中显示给玩家看的文字
-- 不修改 Config 键、Remote 名称、函数名和游戏逻辑。
-- ============================================================
local CN_UI_PHRASES = {
    ["Kích Hoạt"]="激活", ["Kích hoạt"]="激活",
    ["Câu Cá"]="钓鱼", ["Săn Boss"]="狩猎 Boss", ["Quản Lý Cá"]="鱼类管理",
    ["Nhiệm Vụ"]="任务", ["Thần Linh"]="神灵", ["Shop & Chế Mới"]="商店 & 制作",
    ["Dịch Chuyển"]="传送", ["ESP & Thị giác"]="ESP & 视觉", ["Nhân vật"]="角色",
    ["Thử Nghiệm"]="实验功能", ["Cài Đặt"]="设置", ["Nhiệm Vụ Vé"]="任务券",
    ["Đạo Sĩ (Taoist)"]="道士（Taoist）", ["Đạo Sĩ Maoshan"]="茅山道士",
    ["Đảo Khởi Đầu"]="起始岛", ["Đảo Tre"]="竹岛", ["Đảo Tre (Bamboo Isle)"]="竹岛（Bamboo Isle）",
    ["Đảo Chiến Trường"]="战场岛", ["Đảo Thống Trị"]="统治岛",
    ["Đảo Phóng Xạ"]="辐射岛", ["Đảo Phóng Xạ (Fallout Isle)"]="辐射岛（Fallout Isle）",
    ["Đảo Cá Chép"]="鲤鱼岛", ["Đảo Cá Chép (Perch Isle)"]="鲤鱼岛（Perch Isle）",
    ["Đảo Băng Giá"]="冰霜岛", ["Đảo Băng (Bão Tuyết)"]="冰岛（暴风雪）",
    ["Đảo Quả Dừa"]="椰子岛", ["Đảo Quả Dừa (Coconut Isle)"]="椰子岛（Coconut Isle）",
    ["Đảo Hổ Phách"]="琥珀岛", ["Đảo Hổ Phách (Amber Isle)"]="琥珀岛（Amber Isle）",
    ["Đảo Đỉnh Sương Mù"]="迷雾峰岛", ["Đảo Đỉnh Sương Mù (Mistpeak)"]="迷雾峰岛（Mistpeak）",
    ["Vùng Biển Sâu"]="深海区域", ["Vùng Câu Cá Ngầm Lòng Đất"]="地下钓鱼区",
    ["Đấu Trường Boss Realm"]="Boss 领域竞技场", ["Đấu Trường Boss Enzo"]="Enzo Boss 竞技场",
    ["Tìm Server"]="寻找服务器", ["Đổi Server"]="切换服务器", ["Bay Đến"]="飞往",
    ["Lấy Vị Trí"]="获取位置", ["Vị Trí"]="位置", ["Vị Trí 1"]="位置 1", ["Vị Trí 2"]="位置 2", ["Vị Trí 3"]="位置 3",
    ["Vị Trí Nhiệm Vụ"]="任务位置", ["Vị Trí Trở Về"]="返回位置", ["Đảo Hiện Tại"]="当前岛屿", ["Đảo chính"]="主岛",
    ["Bán Cá"]="出售鱼", ["Bán Cá Thành Công"]="出售鱼成功", ["Lỗi Bán Cá"]="出售鱼失败",
    ["Mở Khóa Balo"]="解锁背包", ["Thao Tác Cá"]="鱼类操作", ["Mở Khóa"]="解锁", ["Khóa"]="锁定",
    ["Khóa Lại Mồi"]="锁定鱼饵", ["Mở Khóa Mồi"]="解锁鱼饵",
    ["Mồi Tốt Nhất (Cao Nhất)"]="最佳鱼饵（最高级）", ["Mồi Thấp Nhất (Tiết Kiệm)"]="最低级鱼饵（省钱）",
    ["Tự Dùng Cần Tốt Nhất"]="自动使用最佳鱼竿", ["Tự Dùng Mồi (Auto Bait)"]="自动使用鱼饵",
    ["Tự Dùng Mồi Tốt Nhất"]="自动使用最佳鱼饵", ["Chọn Mồi Khi Câu Thường"]="选择普通钓鱼鱼饵",
    ["Bật Săn Secret Boss (Chat Sniper)"]="开启 Secret Boss 狙击（聊天）",
    ["Tự Động Săn Secret Boss Theo Chat"]="根据聊天自动狩猎 Secret Boss",
    ["Giật Cần Thả Lại (Fast Skip Cá Thường)"]="快速收竿重抛（跳过普通鱼）",
    ["Bỏ Qua Cá Thường (Fast Skip)"]="跳过普通鱼（快速跳过）",
    ["Kiểm Tra Lực Cần (Power Check)"]="检查鱼竿强度（Power Check）",
    ["Chỉ Săn Khi Đủ Lực Cần (Power Check)"]="仅在鱼竿强度足够时狩猎",
    ["Tự Đổi Server Khi Hết Boss (Auto-Hop)"]="Boss 消失后自动换服",
    ["Đổi Server Khi Hết Secret Boss"]="Secret Boss 消失后换服",
    ["Tự Về Vị Trí Farm Khi Hết Boss / Clear"]="Boss 消失 / Clear 后自动返回刷怪位置",
    ["Tự Động Tìm Server Thời Tiết"]="自动寻找天气服务器",
    ["Chọn Thời Tiết Cần Tìm"]="选择目标天气",
    ["Tự Động Câu / Săn Boss Khi Tìm Thấy"]="找到目标后自动钓鱼 / 狩猎 Boss",
    ["Gửi Webhook Khi Tìm Thấy Server"]="找到目标服务器后发送 Webhook",
    ["Tự Động (Theo Acc)"]="自动（按账号）", ["Tự Động (Auto Detect)"]="自动（自动检测）",
    ["Bất Kỳ Thời Tiết Nào (Trừ Clear)"]="任意天气（除晴天）",
    ["Thunderstorm (Bão Sấm)"]="Thunderstorm（雷暴）", ["Rainy (Trời Mưa)"]="Rainy（下雨）",
    ["Windy (Trời Gió)"]="Windy（大风）", ["Snowy (Bão Tuyết)"]="Snowy（暴风雪）",
    ["Foggy (Sương Mù)"]="Foggy（雾）", ["Blazing Sun (Nắng Gắt)"]="Blazing Sun（烈日）",
    ["Clear (Trời Quang)"]="Clear（晴天）",
    ["Chọn Độ Khó Vé Nhiệm Vụ"]="选择任务券难度", ["Chế Độ Nhiệm Vụ"]="任务模式",
    ["Loại Mồi Làm Nhiệm Vụ 100 Mồi"]="100 鱼饵任务所用鱼饵",
    ["Chiêu Dùng Cho Nhiệm Vụ 100 Skill"]="100 次技能任务使用的技能",
    ["Chiêu Giật Nhanh Cho 100 Con Cá"]="100 条鱼快速收竿技能",
    ["Thời Gian Hồi Chiêu (Phút)"]="冷却时间（分钟）",
    ["Tự Bán Cá Khi Đầy Balo (Vé NV)"]="背包满时自动卖鱼（任务券）",
    ["Tự Về Home Spot Khi Xong Nhiệm Vụ"]="任务完成后自动返回 Home 点",
    ["Tự Động Quăng Cần Tại Home Spot"]="自动在 Home 点抛竿",
    ["Nhận & Nộp Vé Từ Xa (Remote)"]="远程领取/提交任务券",
    ["Chọn Vòng Quay Gacha"]="选择 Gacha 转盘", ["Số Vé Mỗi Lần Quay"]="每次转动所用票券数量",
    ["Mẫu Phân Cấp (Preset)"]="优先级预设", ["Ưu Tiên: 🎯 Săn Secret Boss"]="优先：🎯 狩猎 Secret Boss",
    ["Ưu Tiên: 📜 Làm Vé Nhiệm Vụ"]="优先：📜 完成任务券", ["Ưu Tiên: ⛩️ Cúng Thần Linh"]="优先：⛩️ 供奉神灵",
    ["Ưu Tiên: ⚔️ Auto Luyện Chiêu"]="优先：⚔️ 自动练技能", ["Ưu Tiên: 🎣 Treo Farm Thường"]="优先：🎣 普通挂机刷鱼",
    ["ESP Cần Câu Bí Mật"]="ESP 秘密鱼竿", ["ESP Thuyền Bè"]="ESP 船只", ["ESP Đạo Sĩ (Taoist)"]="ESP 道士（Taoist）",
    ["Khiên Nước Axit (Acid Shield)"]="酸液护盾（Acid Shield）", ["Báo Cáo Định Kỳ"]="定时报告",
    ["Báo Cáo Định Kỳ (Mỗi 30 Phút)"]="定时报告（每 30 分钟）", ["Báo Cáo Tiến Độ Mỗi Giờ"]="每小时进度报告",
    ["Thông Báo Hoàn Thành Nhiệm Vụ Vé"]="任务券完成通知", ["Tần Suất Báo Cáo Định Kỳ"]="定时报告频率",
    ["Tần Suất Gửi Báo Cáo"]="报告发送频率", ["Telegram Báo Boss"]="Telegram Boss 通知", ["Telegram Báo Đạo Sĩ"]="Telegram 道士通知",
    ["Thông Báo Đổi Thời Tiết (ntfy)"]="天气变化通知（ntfy）", ["Thông Báo Tìm Server Thời Tiết (ntfy)"]="天气服务器通知（ntfy）",
    ["Thông Báo Boss & NPC (ntfy)"]="Boss & NPC 通知（ntfy）",
    ["Đang quét..."]="正在扫描...", ["Dọn Rác"]="清理垃圾", ["Đã lưu"]="已保存", ["Đang Tắt"]="已关闭",
    ["Đang hoạt động"]="运行中", ["Không có"]="无", ["Không xác định"]="未知", ["Chưa có"]="暂无",
    ["Chưa có cấu hình"]="暂无配置", ["(Chưa có cấu hình)"]="（暂无配置）", ["Chưa nhận nhiệm vụ"]="尚未领取任务",
    ["Không có mô tả"]="暂无描述", ["Không tìm thấy dữ liệu túi đồ!"]="未找到背包数据！",
    ["Không tìm thấy dữ liệu túi đồ người chơi!"]="未找到玩家背包数据！",
    ["Vui lòng chọn một loài cá hợp lệ trước!"]="请先选择有效的鱼类！",
    ["Vui lòng nhập Webhook URL trước!"]="请先输入 Webhook URL！",
    ["Gửi thất bại! Kiểm tra lại URL Webhook."]="发送失败！请检查 Webhook URL。",
    ["MỞ KHÓA THÀNH CÔNG"]="解锁成功", ["[ĐÃ CÓ]"]="[已拥有]", ["[CHƯA CÓ]"]="[未拥有]",
    ["Mặc Định (Gốc)"]="默认（原始）", ["Tùy Biến (Tự Do)"]="自定义（自由）", ["Tùy Biến Thứ Hạng (Custom)"]="自定义等级",
    ["Xuất Kỹ Năng"]="导出技能", ["Gửi Test"]="发送测试", ["Diệt Script"]="关闭脚本",
    ["▼ Thu Gọn"]="▼ 收起", ["▶ Mở Rộng"]="▶ 展开",
    ["Vui lòng nhập tên cấu hình!"]="请输入配置名称！", ["Tên cấu hình không hợp lệ!"]="配置名称无效！",
    ["Lỗi mã hóa dữ liệu cấu hình!"]="配置数据编码错误！", ["Executor của bạn không hỗ trợ hàm writefile!"]="当前执行器不支持 writefile！",
    ["Không thể đọc nội dung file cấu hình!"]="无法读取配置文件！", ["File cấu hình không tồn tại!"]="配置文件不存在！",
    ["File cấu hình bị lỗi định dạng!"]="配置文件格式错误！", ["Không thể xóa file cấu hình!"]="无法删除配置文件！",
    ["Hồi chiêu đã xong! Chuẩn bị nhận vé Hard mới..."]="冷却完成！准备领取新的 Hard 任务券...",
    ["Đã hết nhiệm vụ hôm nay! (Hẹn ngày mai quay lại)"]="今天的任务已完成！（明天再来）",
    ["Đã nộp nhiệm vụ & nhận thưởng vé thành công!"]="任务已提交并成功领取票券奖励！",
    ["Đã nhận thành công nhiệm vụ "]="成功领取任务 ", ["Đang làm: "]="正在进行：",
    ["Đang làm: %s (%d/%d)"]="正在进行：%s（%d/%d）", ["Đang chờ hồi chiêu vé (còn %02d:%02d)"]="等待任务券冷却（剩余 %02d:%02d）",
    ["💤 Đang Chờ (Idle)"]="💤 等待中（Idle）", ["Đã trang bị cần: "]="已装备鱼竿：", ["Đã trang bị mồi: "]="已装备鱼饵：",
    ["Bạn chưa sở hữu cần: "]="你尚未拥有鱼竿：", ["Trong kho bạn có 0 "]="你的仓库中有 0 个",
    ["Mép Nước"]="水边", ["Bán kiếm tiền vàng"]="出售换金币", ["💰 Bán kiếm tiền vàng"]="💰 出售换金币",
    ["💰 Bán tự động dọn trống balo"]="💰 自动出售清空背包",
    ["💰 Bán kiếm tiền nâng cấp"]="💰 出售以赚取升级资金",
    ["💰 Bán kiếm tiền nâng cấp trang bị"]="💰 出售以赚取装备升级资金",
    ["💰 Bán lấy nhiều tiền vàng (Giá trị kinh tế cao)"]="💰 出售高价值鱼以赚取更多金币",
    ["💰 Tiền Hiện Tại"]="💰 当前金币", ["💎 Gems Hiện Có"]="💎 当前 Gems", ["🎫 Vé Nhiệm Vụ"]="🎫 任务券",
    ["Nguyên liệu chính"]="主要材料", ["Nguyên liệu phụ"]="辅助材料",
    ["Chế Cần Huyết Long"]="制作血龙鱼竿", ["Chế Cần Huyết Long Rod"]="制作血龙鱼竿",
    ["Chế Cần Heavenpiercer Rod"]="制作 Heavenpiercer 鱼竿", ["Chế Cần Pure Diamond Rod"]="制作 Pure Diamond 鱼竿",
    ["Chế Mồi Rainbow Bait"]="制作 Rainbow 鱼饵", ["Mồi Rainbow Bait"]="Rainbow 鱼饵", ["Mồi Nameless Bait"]="Nameless 鱼饵",
    ["Cần Heavenpiercer Rod"]="Heavenpiercer 鱼竿", ["Cần Heavenpiercer & Mồi Rainbow"]="Heavenpiercer 鱼竿 & Rainbow 鱼饵",
    ["Cần Pure Diamond Rod"]="Pure Diamond 鱼竿", ["Boss Bạch Tuộc (Phao Biển)"]="章鱼 Boss（海上浮标）",
    ["Cá Secret Boss"]="Secret Boss 鱼", ["Cá Đột Biến"]="变异鱼", ["Cá Cần Giữ"]="需要保留的鱼", ["Cá Quý Hiếm"]="稀有鱼",
    ["Không có bão"]="无风暴", ["Đang bận xử lý tác vụ khác, vui lòng đợi!"]="正在处理其他任务，请稍候！",
    ["Bán Cá Thành Công"]="出售鱼成功", ["Không tìm thấy Remote SellFish!"]="未找到 Remote SellFish！",
    ["Tìm Kiếm"]="搜索", ["Đã bay đến người chơi: "]="已飞往玩家：", ["🔄 Làm Mới"]="🔄 刷新",
    ["Quest Phụ"]="支线任务", ["Đã dịch chuyển đến vị trí "]="已传送至位置 ",
    ["Mở Khóa Index"]="解锁图鉴", ["Ảo Hoá Vé"]="虚拟任务券", ["Ảo Hoá Gems"]="虚拟 Gems",
    ["Nạp Thiết Yếu"]="加载核心设置", ["Lưu Config"]="保存配置", ["Nạp Config"]="加载配置", ["Xóa Config"]="删除配置",
    ["1 (Cao Nhất)"]="1（最高）", ["5 (Thấp Nhất)"]="5（最低）", ["Thông Báo Bắt Được Boss"]="Boss 捕获通知",
    ["Đạo Sĩ Mao Sơn"]="茅山道士", ["Đại Dương"]="大海"
}

local function CN_TranslateText(raw)
    if type(raw) ~= "string" or raw == "" then return raw end
    local out = raw
    for k, v in pairs(CN_UI_PHRASES) do
        out = out:gsub(k:gsub("([^%w])", "%%%1"), v)
    end
    return out
end

local function CN_TranslateGui(root)
    if not root then return end
    pcall(function()
        for _, obj in ipairs(root:GetDescendants()) do
            if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                local old = obj.Text
                local new = CN_TranslateText(old)
                if new ~= old then obj.Text = new end
                if obj:IsA("TextBox") then
                    local ph = obj.PlaceholderText
                    local newPh = CN_TranslateText(ph)
                    if newPh ~= ph then obj.PlaceholderText = newPh end
                end
            end
        end
    end)
...
-- ============================================================
-- V5 LANGUAGE SWITCH
-- 中文默认 / English / Tiếng Việt
-- 仅处理本脚本 ScreenGui 的显示文本，不修改功能逻辑。
-- ============================================================
pcall(function()
    if not screenGui or not titleBar then return end

    local oldLang = screenGui:FindFirstChild("V4LanguageButton", true)
    if oldLang then oldLang:Destroy() end

    local langButton = Instance.new("TextButton")
    langButton.Name = "V4LanguageButton"
    langButton.Size = UDim2.new(0, 52, 0, 24)
    langButton.Position = UDim2.new(1, -118, 0, 7)
    langButton.BackgroundColor3 = Colors.ControlBg
    langButton.TextColor3 = Colors.PurplePrimary
    langButton.Font = Enum.Font.GothamBold
    langButton.TextSize = 10
    langButton.Text = "中文"
    langButton.BorderSizePixel = 0
    langButton.ZIndex = 100
    langButton.Parent = titleBar
    Instance.new("UICorner", langButton).CornerRadius = UDim.new(0, 4)

    local lang = "zh"
    local reverseCN = {}
    for vi, zh in pairs(CN_UI_PHRASES or {}) do
        if type(vi) == "string" and type(zh) == "string" and zh ~= "" then
            if not reverseCN[zh] then reverseCN[zh] = vi end
        end
    end

    local EN = {
        ["钓鱼"]="Fishing", ["狩猎 Boss"]="Boss Hunt", ["鱼类管理"]="Fish Manager", ["任务"]="Quests",
        ["神灵"]="God Spirit", ["商店 & 制作"]="Shop & Craft", ["传送"]="Teleport", ["ESP & 视觉"]="ESP & Visuals",
        ["角色"]="Player", ["实验功能"]="Experimental", ["设置"]="Settings", ["任务券"]="Quest Tickets",
        ["搜索"]="Search", ["刷新"]="Refresh", ["出售鱼"]="Sell Fish", ["出售鱼成功"]="Fish Sold",
        ["解锁"]="Unlock", ["锁定"]="Lock", ["运行中"]="Running", ["暂无"]="None", ["未知"]="Unknown",
        ["关闭脚本"]="Stop Script", ["收起"]="Collapse", ["展开"]="Expand", ["已保存"]="Saved", ["已关闭"]="Off",
        ["自动寻找天气服务器"]="Auto Find Weather Server", ["寻找服务器"]="Find Server", ["切换服务器"]="Server Hop",
        ["当前金币"]="Current Coins", ["当前 Gems"]="Current Gems", ["任务完成通知"]="Quest Complete Notification",
        ["天气变化通知（ntfy）"]="Weather Change Notification (ntfy)", ["天气服务器通知（ntfy）"]="Weather Server Notification (ntfy)",
        ["Boss & NPC 通知（ntfy）"]="Boss & NPC Notification (ntfy)", ["水边"]="Water Edge", ["大海"]="Ocean"
    }

    local function replaceAll(text, from, to)
        if not text or text == "" or from == "" then return text end
        local patt = from:gsub("([^%w])", "%%%1")
        return text:gsub(patt, function() return to end)
    end

    local function convertText(text, target)
        if type(text) ~= "string" or text == "" then return text end
        local out = text
        if target == "vi" then
            for zh, vi in pairs(reverseCN) do
                out = replaceAll(out, zh, vi)
            end
        elseif target == "en" then
            for zh, en in pairs(EN) do
                out = replaceAll(out, zh, en)
            end
        end
        return out
    end

    local function translateGui(target)
        for _, obj in ipairs(screenGui:GetDescendants()) do
            if obj == langButton then
                -- handled below
            elseif obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                local t = obj:GetAttribute("V4BaseText")
                if t == nil then
                    t = obj.Text
                    obj:SetAttribute("V4BaseText", t)
                end
                local nt = t
                if target == "zh" then
                    nt = t
                    -- base script is already Chinese in V4; apply known Vietnamese->Chinese mapping too
                    nt = CN_TranslateText(nt)
                elseif target == "vi" then
                    nt = convertText(t, "vi")
                else
                    nt = convertText(CN_TranslateText(t), "en")
                end
                if obj.Text ~= nt then obj.Text = nt end
                if obj:IsA("TextBox") then
                    local ph = obj:GetAttribute("V4BasePlaceholder")
                    if ph == nil then ph = obj.PlaceholderText; obj:SetAttribute("V4BasePlaceholder", ph) end
                    local np = ph
                    if target == "vi" then np = convertText(ph, "vi") elseif target == "en" then np = convertText(CN_TranslateText(ph), "en") else np = CN_TranslateText(ph) end
                    if obj.PlaceholderText ~= np then obj.PlaceholderText = np end
                end
            end
        end
    end
    ...
    langButton.MouseButton1Click:Connect(function()
        if lang == "zh" then lang = "en"
        elseif lang == "en" then lang = "vi"
        else lang = "zh" end
        translateGui(lang)
        langButton.Text = (lang == "zh" and "中文") or (lang == "en" and "EN") or "VI"
    end)

    -- 立即中文化，并持续处理后续动态生成的按钮/提示。
    translateGui("zh")
    task.spawn(function()
        while isRunning and screenGui and screenGui.Parent do
            pcall(function()
                -- 强制保持悬浮头像隐藏，防止最小化/其他逻辑再次把它显示出来
                if floatingAvatar then floatingAvatar.Visible = false end
                translateGui(lang)
            end)
            task.wait(0.8)
...
-- V5 final safety: remove any floating avatar/image GUI created by this script.
pcall(function()
    if floatingAvatar then
        floatingAvatar.Visible = false
        floatingAvatar.Active = false
    ...
    if screenGui then
        for _, obj in ipairs(screenGui:GetDescendants()) do
            if obj:IsA("ImageButton") or obj:IsA("ImageLabel") then
                ...
                if n:find("avatar", 1, true) or n:find("floating", 1, true) then
                    obj.Visible = false
                    obj.Active = false
